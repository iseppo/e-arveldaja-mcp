...
127 tools by default on the compatibility-preserving `standard` profile; opt-in `guided` / `guided-sales` expose 19 / 20 tools, and `full` exposes all 147. The prompt registry holds 16 workflow prompts (guided profiles register fewer — a prompt whose required tools are all absent is not registered) and there are 15 resources across 12 modules. Supports multiple companies/accounts.
...
Connection names are unique: when two distinct credentials derive the same name
(e.g. `apikey.txt` + `apikey.TXT`, or `env-local` from two working dirs), later ones
in load order get a deterministic `-2`, `-3`, … suffix, so a name-pinned call is never ambiguous.
...
under `EARVELDAJA_RULES_FILE` single-file mode). Because a bundle can be shared
by several connections (`EARVELDAJA_RULES_DIR`, or a kept-in-place project/global
bundle), the record — like each `statement-balances.json` record — is stamped with
the active connection fingerprint and ignored (with a warning) on another
connection; a legacy unstamped record applies only when exactly one connection is
configured. Both files are written atomically 0600 and reads are capped at 1 MB. At compute time the stored
...
`EARVELDAJA_PROFILE` provides one-choice surfaces: `guided` (19 merged daily-bookkeeping tools), `guided-sales` (those 19 plus the `manage_sale_invoice` sales façade — read plus the full prepare/execute mutation lifecycle: create/update/delete/confirm/invalidate/send/recurring and inline resolve-or-create customer, 20 total), `standard` (the absent-variable compatibility default, 127), or `full` (all 147, including granular/setup tools and the workflow-state paging tool `get_workflow_page`). Guided profiles are opt-in for this release. Any explicitly present legacy exposure flag forces the normalized profile to `custom` and preserves the flag combination below. Profile and catalog fingerprint are part of plan/file-reference scope: restart and obtain a fresh preview after changing them.

The guided surface is sized for routing clarity, not token cost (per-tool measurement shows no cost cliff at 20). The **≤ 20** figure is a target, not a hard wall: 21–24 is a deliberate-exception band (allowed only with a recorded `CHANGELOG` rationale when no existing tool can absorb the capability), and 24 is the hard backstop above which the surface must be redesigned by merging tools rather than adding. Before adding any guided tool, apply the add/don't-add criteria in [`docs/guided-tool-policy.md`](docs/guided-tool-policy.md) — prefer a new `mode` on an existing façade, a `continue_accounting_workflow` continuation, or `standard`/`full` placement (as with `get_server_status`) over growing the daily surface.

`get_server_status` is a compact read-only tool (version, active profile, and any active point-of-use release notices) registered unconditionally like `get_setup_instructions`; it needs no credentials and belongs to no opt-out group, so it is visible in `standard`/`full` (and setup mode) but not the guided surfaces. The fixed per-session `instructions` string is kept lean (built in `src/server/server-instructions.ts`, under 1.5 KiB; its entry-point tool names are profile-aware — the guided façades on `guided`, the granular entry points on `standard`): detailed VAT / bank-direction / reporting guidance lives in the owning `workflows/*.md` prompts, and the v0.22.0 incoming-direction regression advisory is a point-of-use release notice (`src/server/release-notices.ts`) surfaced only at the start of the affected bank flows, not in the global text.
...
- **`EARVELDAJA_EXPOSE_GRANULAR_TOOLS=1`** — also register the 10 granular
  ...
  `reconcile_inter_account_transfers` is not granular-gated and stays registered
  on `standard`/`full`; the merged `reconcile_bank_transactions` also executes
  inter-account transfers via `mode="execute_inter_account"` (the only route on
  the guided profiles).
...
MCP workflow prompts are profile- and exposure-aware too: `registerPrompts`
derives the registered tool set (`promptToolAvailability` in
`src/prompt-registry.ts`, the same derivation the server `instructions` use),
renders a `workflows/*.md` feature section only when its tools are registered
(e.g. the `standard` granular flow vs the `guided` façade flow, the `sales`
extension), and does not register a prompt whose `requiredTools` are all absent
(e.g. `lightyear-booking` / `new-supplier` on `guided`).
`src/prompt-tool-surface.test.ts` pins, for every profile, that rendered
prompts and instructions name only registered tools. The static
`.claude/commands/*.md` mirrors are profile-agnostic: they contain every
section, each wrapped in a capability condition ("run only when these tools are
advertised …") so the agent skips sections whose tools are not in `tools/list`.
...
- **Always use `reconcile_inter_account_transfers`** (or `reconcile_bank_transactions` `mode="execute_inter_account"`) for inter-account confirmation — it checks existing journals before confirming
...
- **ÄS § 157 legality checks are split by clause**: the **lg 1 ceiling is NET-based** — the ENTIRE lg 1 distributable profit (retained earnings 2960 + closed prior-year result on "Aruandeaasta kasum" 2970, name-resolved + unclosed prior-year P&L; never the distribution year's own result) is distributable as net dividend, since the CIT is not part of the payout; the **lg 2 net-assets floor is GROSS-based** (net assets fall by dividend payable + tax liability) at share capital **+ non-distributable reserves** (reservkapital, name-detected in the chart, standard 2940; override with `restricted_reserve_accounts`). Live unconfirmed dividend drafts (`DIV-…` or crediting dividend payable) are subtracted from both headrooms, since the tool books drafts and balances read only confirmed journals. The tool hard-blocks violations of either clause unless `force=true` (use only alongside a legitimate action such as a capital reduction; `force` never overrides the ledger-imbalance block), and always reports `maximum_distributable.max_net_dividend` = min(lg 1 distributable profit, (net assets − floor) × den/(den+num)), both net of pending drafts. One journal per shareholder+date: an identical retry returns `duplicate`, a different amount `dividend_key_conflict`.
- **Year-end close (RIK method, calendar year).** e-arveldaja has no native closing; RIK's guide "Äriühingu majandusaasta lõpetamiskanded e-arveldajas" prescribes two manual entries, which `prepare_year_end_close` / `execute_year_end_close` propose and book as drafts: (1) Dec 31 profit D 9000 "Arvestuslik koondtulemus" / K 2970 "Aruandeaasta kasum" (loss reversed), document `YEC-RESULT-YYYY`; (2) Jan 1 of the next year D 2970 / K 2960 (optional `reserve_capital_amount` to 2940), document `YEC-RETAINED-YYYY`. Revenue/expense accounts are NOT zeroed and 9000 is not part of the income statement: every P&L surface (`compute_profit_and_loss`, `run_accounting_report`, `generate_annual_report_data`, prepare's result) excludes it, while the balance sheet keeps it in open P&L so equity balances. Existing entries are detected structurally (date + accounts touched, any document number, drafts included); legacy `YECL-YYYY` journals (the old zero-all-P&L close, `src/year-end-closing-journal.ts`) that post to 2970 count as entry 1 — never book either twice. Entry 2 is 2970 against 2960 and/or reserve accounts only (an all-to-reserve D 2970 / K 2940 counts); it is also recognised on any date of the next year when its amount matches (operators often transfer on another day). Both entries are checked for direction and amount; a mismatch is reported as `mismatch`/`partially_closed` in `blocked_entries` and blocks booking — `allow_additional_transfer` acknowledges only a smaller same-direction 1 January transfer (the remainder is booked); an off-date, ambiguous, over- or reversed transfer and a wrong entry 1 always need manual correction. The dividend §157 lg 1 unclosed-P&L sum must INCLUDE 9000 (it cancels the still-open P&L once entry 1 exists).
- **Dividend dry run**: `prepare_dividend_package` defaults to `dry_run=true` (preview only); call again with `dry_run=false` only after the user explicitly approves the previewed journal.
...
- **Capital gains (company)**: securities gains are ordinary income in the P&L (Lightyear sells: gain balancing to 8330/8335); there is no tax at realisation — Estonian CIT is deferred until profit is distributed (dividend / TuMS § 50)
- **Third-party payer / receipt client alignment**: e-arveldaja copies a registration journal's `clients_id` from the bank transaction (payer), not from the linked invoice, and postings carry no client — so a payer that differs from the invoice client books the receivable credit into the wrong client sub-ledger (verified live 2026-09). `TransactionsApi.confirm` refuses such a confirm (`linked_invoice_client_mismatch`) unless `reassignClientToInvoice` (tool param `reassign_client_to_invoice`, an explicit approval) is set; a distribution linking invoices of several clients is always refused (`linked_invoice_clients_ambiguous`, no reassignment can fix one journal/one client — split the payment); the registration journal is located by `operation_type = "TRANSACTION"` + `operations_id = tx id` (the register call returns no journal id) and checked after every invoice-linked confirm (`src/banking/receipt-ledger-check.ts`); exact-match reconciliation routes these to `third_party_payer_reviews`; `run_accounting_report` `receipt_client_alignment` audits existing books. Repair is invalidate → reassign → re-confirm; a one-journal reclassification between two clients is not expressible.
