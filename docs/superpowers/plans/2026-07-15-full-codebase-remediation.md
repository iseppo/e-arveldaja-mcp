...
#### M23 quality-review correction plan

The first frozen implementation artifact passed specification review but did not pass code-quality review. Four confirmed findings invalidate that artifact and both review verdicts: the mutable connection label changed the selected path for an unchanged fingerprint, the identity suffix was only 32 bits, a throwing live getter silently fell back to the shared `default` scope, and bundle-root inspection errors were treated as absence. Correct them sequentially before M23 can be committed:

1. Add four tests and strengthen existing M23 tests before production edits. Require different labels with the same stable identity to resolve to the same scope, different identities to remain isolated, and the canonical scope to be a bounded 64-character lowercase SHA-256 digest. Extend the connection-switch test through a label-only rename, add a throwing-getter fail-closed test, add an explicit `setup` getter passing control, make explicit file/dir overrides bypass even a throwing getter, and add separate project/global inspection-error tests using a regular file at `accounting-rules/auto-booking` to deterministically trigger `ENOTDIR`. Expected pre-fix result: 16 M23 tests discovered, 5 FAIL / 11 PASS; whole file 5 FAIL / 57 PASS.
2. Make `buildAccountingRulesConnectionScope` derive the canonical path component only from the full SHA-256 digest of the non-secret stable identity. Connection labels remain human-facing metadata and `sanitizeAccountingRulesConnectionName` remains a safe exported utility, but labels no longer determine storage identity. Reject a blank stable identity. This keeps the scope within one portable 64-character component and makes label changes path-stable without directory discovery.
3. Preserve explicit `EARVELDAJA_RULES_FILE` and `EARVELDAJA_RULES_DIR` precedence before evaluating the live getter. Outside those overrides, wrap getter failures in a descriptive error with the original cause and abort resolution; never read or write a shared fallback. The index-provided `{ name: "setup", stableIdentity: "setup" }` remains the explicit setup path.
4. Make root-bundle probing conservative: absent remains false and a successfully inspected empty directory remains false, but an existing non-directory root or any inspection error pins that project/global root so it cannot be stranded by selecting a fresh scoped store.
...
#### M26 third review-correction plan

The 33,156-byte replacement artifact passed specification review but failed code-quality review for two P1 defects. First, the new public trade DTO still emits CSV-controlled `date` and `currency` strings raw. Second, a finite but unbounded quantity or individually/cumulatively unsafe money arithmetic can enter WAC, serialize a quantity as `null`, or make `roundMoney` throw on a non-finite aggregate. Invalidate that artifact and every verdict, then correct both defects before another review:

1. Strengthen the existing M26 unsafe-imported-identifiers test without changing its count. Give ready, skipped, and review-required rows noncanonical date/currency strings containing a newline and instruction text; assert one `UNTRUSTED_OCR` start/end pair per unsafe public value. Preserve exact safe controls for a real canonical `YYYY-MM-DD` date and uppercase three-letter `EUR`/`USD` currencies. Add private output-only renderers: a date stays raw only when it is an exact, real calendar `YYYY-MM-DD`, and a currency stays raw only when it matches `[A-Z]{3}` exactly; wrap every other non-empty value once. Apply them only in `portfolioTradeBaseDto`, after all raw parsing, sorting, matching, FX logic, and cash-equivalent routing.
2. Add four new M26 tests before production edits. The first supplies one coherent EUR trade whose finite quantity is `Number.MAX_VALUE`; both portfolio and booking dry-run must return the same stable `trade_amount_conflict`, with no `booked_basis`, WAC position, total, journal, or throw. The second supplies individually safe, large exact-cent buys for the same ticker whose combined holding cost crosses the supported bound; the first acceptable row remains previewed and the crossing row is rejected without changing quantity, cost, buy count, or accepted basis. The third uses separate tickers whose individually safe holdings would cross the portfolio-wide remaining-cost bound; reject the crossing row so the exposed total can never overflow even when every holding remains individually valid. The fourth creates a valid holding and accepted sell, then presents a later individually safe sell that would overflow cumulative proceeds, cost, or realized arithmetic; rejection must preserve quantity, remaining cost, proceeds, realized gain/loss, sell count, accepted basis, and original review ordering. Every cumulative rejection uses exact new `portfolio_arithmetic_overflow`, is absent from `booked_basis` and WAC, and returns finite JSON without throwing. Together with the strengthened existing unsafe-identifier test, expected test-only RED is M26 5 FAIL / 12 PASS and Lightyear 5 FAIL / 324 PASS out of 329; no transform/import-only failure is acceptable.
3. Extend `FxReviewCode`/`FX_REVIEW_MESSAGES` with `portfolio_arithmetic_overflow` for portfolio-only cumulative rejection. Strengthen the shared intrinsic classifier input to include `type`, `quantity`, and `fx_fee_eur`. Before returning ready, require a positive finite quantity representable as a safe integer at the existing six-decimal public precision; require `eur_amount`, converted trade fee, FX fee, and every type-relevant posting arithmetic combination to be exact bounded cents via `moneyToSafeCents` and safe-integer cent addition/subtraction. Return `trade_amount_conflict` for any individual quantity or arithmetic violation. Keep an existing extraction reason first and unresolved foreign provenance/fee precedence unchanged. Both `book_lightyear_trades` and portfolio must continue consuming this one classifier, so individual unsafe rows have exact booking/portfolio parity and booking never calls `roundMoney` on their unsafe arithmetic.
4. Replace eager `readyTrades` publication with candidate processing and an `acceptedReadyTrades` list. For each intrinsically ready candidate, calculate the proposed holding in a fresh value rather than mutating the stored holding. Preserve the exact legacy floating-point WAC formulas and stored values for every accepted trade; use bounded-cent conversion only as a validation view and for the already-cent-rounded exposed totals, never as replacement arithmetic for quantity, average cost, sold cost, remaining cost, proceeds, or realized gain/loss. Validate the proposed six-decimal quantity; bounded-cent remaining cost, proceeds, realized gain/loss, sold cost, and average-cost intermediates; and checked portfolio-wide remaining-cost and realized totals. Maintain each exposed global total by safely replacing the prior holding's rounded cents with the proposed holding's rounded cents, which matches the legacy sum-of-rendered-positions contract without an unchecked final `reduce`. Commit the proposed holding and append the trade to `acceptedReadyTrades` only after every check passes. On failure, leave the prior holding and portfolio totals byte-for-byte unchanged, add the trade to `review_required` with `portfolio_arithmetic_overflow`, and continue. Preserve statement order inside every public bucket by retaining the original trade ordinal when late WAC rejection is recorded. Derive `booked_basis` only from `acceptedReadyTrades`; build positions and aliases from the already-validated state; render final totals from the maintained checked cent aggregates.
5. Capture the four new focused RED tests before implementation. Then require M26 17/17, H16 80/80, H17 196/196, H18 22/22, and Lightyear 329/329. Re-prove all four protected slices byte-identical, `npm run build`, `npm run validate:release`, `git diff --check`, full unit 2,474/2,474, and integration 20 pass/3 documented skips. Require exactly the same two M26 files and no M23/H08 drift.
6. Regenerate `.omc/reviews/M26.diff`, record new bytes/SHA-256, and prove live identity. Restart ordered fresh specification review, then a different fresh code-quality review, then an independent verifier. Reviews must explicitly cover safe date/currency controls and exact-once wrapping; individual booking parity; cumulative rejection atomicity and ordering; no partial mutation; finite JSON; bounded position and cross-position totals; accepted-basis derivation; protected behavior; test strength; and exact scope. Any edit or finding invalidates the artifact and restarts all gates and reviews.

#### M26 fourth review-correction plan

The 49,790-byte artifact is unapproved after specification review. One finding exposed an over-constrained sentence in the third correction plan rather than a production regression: the conservative `MAX_UNAMBIGUOUS_MONEY_CENTS` portfolio ceiling cannot also be the individual booking ceiling because protected H16 intentionally books `35,575,111,755,115.99` EUR while its cents and posting sums remain exact safe integers. Two further findings require a public quantity-alignment fix and stronger controls:

1. Correct the individual-versus-aggregate contract. Shared intrinsic readiness validates `eur_amount`, converted trade fee, FX fee, and type-specific posting sums through exact, round-tripping safe-integer cents (`moneyToPostingSafeCents` plus checked integer addition/subtraction), without the stricter 10-trillion-euro portfolio ceiling. Preserve the protected H16 amount as intrinsically ready and booking-ready; add it as a direct classifier compatibility control. Apply `moneyToSafeCents` and `MAX_UNAMBIGUOUS_MONEY_CENTS` only to prospective WAC fields and exposed portfolio totals. A single exact posting above that analytical ceiling may therefore remain booking-ready while portfolio reports `portfolio_arithmetic_overflow`; this is the intended intrinsic-versus-analytical boundary, not booking divergence.
2. Preserve raw fractional quantities internally but align public portfolio precision. Do not reject a finite positive quantity solely for having more than six decimals. Extraction, booking, gains matching, and legacy floating WAC continue using the raw value. Add one shared safe-microunit conversion for public rendering and WAC validation. In `portfolioTradeBaseDto`, render a safely scalable quantity at six decimals, matching `quantity_held`; if a skipped or review-required finite quantity cannot be safely scaled, retain its finite raw value rather than deriving `Infinity`/`null`. Change the existing `1.0000001` compatibility control to require classifier/booking readiness and exact public `booked_basis.quantity === previewed.quantity_held === 1`. This is the only intended production change in this correction.
3. Strengthen existing tests without increasing the 17-test M26 count. In the unsafe-output test, assert a normal H16-ready USD trade preserves exact `currency: "USD"` and canonical `YYYY-MM-DD`, alongside the already-required unsafe wrapped currency/date cases. In the late-sell/ordering test, use an earlier intrinsic rejection, accepted large trade, later WAC rejection, and final intrinsic rejection; require exact `review_required` reference order by the sorted trade traversal. This makes the ordinal merge load-bearing. The test-only quantity expectation must produce exact RED M26 1 FAIL / 16 PASS and full Lightyear 1 FAIL / 328 PASS out of 329; the added USD, H16 amount, and ordering controls must already pass. No import/transform failure is acceptable.
4. After the minimal quantity-rendering fix, require M26 17/17, H16 80/80, H17 196/196, H18 22/22, Lightyear 329/329, all four protected-slice hashes unchanged, build, release validation, `git diff --check`, full unit 2,474/2,474, and integration 20 pass/3 documented skips. Regenerate the exact two-file artifact and restart fresh ordered specification and code-quality reviews followed by an independent verifier. Any edit or finding invalidates all evidence and verdicts again.

#### M26 fifth review-correction plan

The 51,461-byte artifact is unapproved because the classifier requires rounded public microunits to be positive. That incorrectly rejects a valid raw positive quantity such as `0.0000004`, even though the fourth correction explicitly limits six-decimal rounding to public rendering:

1. Extend the existing M26 classifier/quantity compatibility test without increasing the 17-test count. Direct classification of a coherent EUR buy with raw quantity `0.0000004` must be ready. Add four coherent EUR buys for one ticker, each quantity `0.0000004` and cost 1 EUR. Booking dry-run must keep all four raw-positive trades as `would_create`; each `booked_basis.quantity` renders as `0`, while raw WAC accumulates `0.0000016` before final rendering and therefore yields one active preview with `quantity_held: 0.000002`, `remaining_cost_eur: 4`, `avg_cost_per_unit: 2500000`, and `buys: 4`. Rounding each input before WAC would fail this control. Expected test-only RED is M26 1 FAIL / 16 PASS and Lightyear 1 FAIL / 328 PASS out of 329.
2. In the shared classifier remove only the requirement that rounded microunits be greater than zero. Keep raw `quantity > 0`, finiteness, and safe-integer scaled rounding. Do not change raw extraction, booking, matching, WAC arithmetic, public trade rendering, or final preview rounding.
3. Require M26 17/17, H16 80/80, H17 196/196, H18 22/22, Lightyear 329/329, unchanged protected slices, build, release validation, `git diff --check`, full unit 2,474/2,474, and integration 20 pass/3 skips. Regenerate the two-file artifact and restart all ordered reviews and independent verification; any edit/finding invalidates the new artifact.
...
#### H08 execution-plan hardening

The original one-test sketch is insufficient because every current import-tool fixture has no bank-account records, `accounts_dimensions_id` may have multiple bank-account rows, and the guard must protect dry-run and merged entry points before any transaction/client work. Execute this stronger contract:

1. Record the clean baseline: `src/tools/camt-import.test.ts` plus `src/tools/camt-import-tools.test.ts` is 29/29 and full unit is 2,474/2,474. Update only the tools-test setup to provide one matching dimension-7 bank account by default; allow each test to override `bankAccounts`. This fixture-only compatibility change must leave all 29 existing tests green before adding H08 assertions.
2. Add nine H08-tagged tests without production edits, keeping all three listed files in scope. In `camt-import.test.ts`, add one PASS control proving the parser preserves a whitespace/lowercase statement IBAN as metadata for later normalization. In `camt-import-tools.test.ts`, add three PASS controls: normalized whitespace/case match via `iban_code`, fallback match via `account_no` when `iban_code` is blank, and matching any of multiple records bound to the selected dimension. Add five RED cases: execute mismatch where another dimension owns the statement IBAN, the same mismatch through `process_camt053` dry-run, an existing selected dimension with no bank-account row, selected bank-account rows with no usable `iban_code` or `account_no`, and a newline/instruction-bearing mismatched statement identity whose error contains one untrusted wrapper rather than exposing unsafe raw text. Every RED must prove `transactions.listAll`, client lookup, transaction create, progress, and other mutation work did not run; mismatch errors name the selected dimension and its safe configured identities, and identify another owning dimension when known. Expected H08 selector: 5 FAIL / 4 PASS; both files: 5 FAIL / 33 PASS out of 38. No import/transform-only failure is valid RED.
3. Add one normalized account-identity helper and export `assertStatementAccountMatchesDimension(api, statementIban, dimensionId)`. Normalization removes all whitespace and uppercases without stripping other characters. Fetch bank accounts once; inspect every record with the selected `accounts_dimensions_id`; accept when normalized statement identity equals any nonblank normalized `iban_code` or `account_no`. If no row is bound, if bound rows have no usable identities, or if none matches, throw an explicit validation error. For diagnostics, deduplicate selected identities deterministically, detect matching other-dimension rows, keep canonical `[A-Z0-9]{1,34}` identities plain, and wrap every noncanonical external statement/API identity exactly once with `wrapUntrustedOcr`. Do not log, mutate, or silently choose another dimension.
4. Reorder only `import_camt053`: validate date arguments, parse the file once to obtain `metadata.iban`, validate the selected dimension still exists, then call the new binding guard. Only after it succeeds may `enrichWithDuplicates`, either `transactions.listAll`, client resolution, progress, result construction, create, or audit occur. Apply the same handler behavior to dry-run, execute, and `process_camt053` delegation; parse-only mode remains read-only and does not require an e-arveldaja bank-account binding. Preserve date filtering, duplicate/client semantics, response shapes, and H09 scope.
5. Expected GREEN: H08 9/9 and the two files 38/38. Then run build, release validation, `git diff --check`, full unit 2,483/2,483, and integration 20 pass/3 documented skips. Freeze an exact three-file H08 artifact and obtain fresh ordered specification review, code-quality review, and independent verification. Any edit/finding invalidates the artifact and restarts the gates/reviews.

#### H08 first review-correction plan

The 15,430-byte artifact passed specification review but failed code-quality review for two P1 trust-boundary gaps. Invalidate the artifact and verdicts before correction:

1. Add two H08-tagged tests before production edits. One configures the normalized statement identity on both the selected dimension and another valid dimension; duplicate rows inside the selected dimension remain harmless, but cross-dimension ownership must reject as ambiguous before transaction listing, client lookup, progress, create, or audit. The second includes a valid selected-dimension row with a different configured identity plus a statement-matching bank-account row whose runtime `accounts_dimensions_id` is malformed, such as a string containing newline/instruction text. This forces current production through its unsafe other-owner diagnostic rather than the already-safe no-selected-row branch. The import must fail closed with `validation_failed`, must not expose that raw value outside an untrusted wrapper (or interpolate it at all), and must have the same zero-side-effect proof. Expected test-only RED: H08 2 FAIL / 9 PASS and combined CAMT 2 FAIL / 38 PASS out of 40; no import/transform failure is valid.
...
#### H08 third review-correction plan

The 20,997-byte artifact failed specification review because `renderAccountIdentity()` tests the normalized value and therefore emits a noncanonical external API identity such as `ee63 7700 7710 1121 2909` as plain canonical text. Invalidate that artifact and all verdicts. Keep normalization exclusively for identity comparison; diagnostic rendering must classify the original value:

1. In `src/tools/camt-import-tools.test.ts`, add one H08-tagged test before any production edit. Use canonical mismatched statement identity `EE111111111111111111` and one selected-dimension bank-account row whose `iban_code` is the noncanonical external value `ee63 7700 7710 1121 2909` and whose `account_no` is blank. Catch the `validation_failed` mismatch error and prove that the statement identity and selected dimension remain plain, while the configured identity appears inside exactly one nonce-matched `<<UNTRUSTED_OCR_START:...>>` / `<<UNTRUSTED_OCR_END:...>>` pair. Assert the raw configured value does not occur in the `configured identities:` diagnostic outside that wrapper, and call `expectNoH08ImportSideEffects(api)` to preserve the transaction/client/progress/create/audit boundary. Do not weaken the existing successful lowercase/whitespace matching controls.
2. Prove the focused RED before touching `src/tools/camt-import.ts`. Run `npx vitest run src/tools/camt-import-tools.test.ts -t "H08"`; expected: 1 FAIL / 11 PASS, with only the new configured-identity wrapping assertion failing because current rendering exposes normalized `EE637700771011212909` as plain text. Run `npx vitest run src/tools/camt-import.test.ts src/tools/camt-import-tools.test.ts`; expected: 1 FAIL / 41 PASS out of 42. A parser, fixture, import, or side-effect failure is not valid RED.
3. In `src/tools/camt-import.ts`, change only `renderAccountIdentity(value)`: return `value` unchanged only when the original string itself matches `^[A-Z0-9]{1,34}$`; otherwise return `wrapUntrustedOcr(value) ?? value`. Do not call `normalizeAccountIdentity()` from the renderer. Leave ASCII-only normalization, comparison semantics, selected-identity deduplication, cross-dimension ownership checks, malformed-owner handling, validation categories, call order, and all mutation guards unchanged. This means canonical original statement/API identities remain plain, while lowercase, whitespace-formatted, non-ASCII, or otherwise noncanonical external identities are wrapped exactly once without being rewritten for display.
4. Prove GREEN in order. Run `npx vitest run src/tools/camt-import-tools.test.ts -t "H08"` and require 12/12; run `npx vitest run src/tools/camt-import.test.ts src/tools/camt-import-tools.test.ts -t "H08"` and require H08 13/13 across both files; run `npx vitest run src/tools/camt-import.test.ts src/tools/camt-import-tools.test.ts` and require 42/42; run `npm run build`, `npm run validate:release`, `git diff --check`, `npm test`, and `npm run test:integration`. Require full unit 2,487/2,487 and integration 20 pass/3 documented skips. Confirm `git status --short` and `git diff --name-only` show exactly `src/tools/camt-import.ts`, `src/tools/camt-import.test.ts`, and `src/tools/camt-import-tools.test.ts`; no H09 work may enter the artifact.
5. Regenerate the exact three-file H08 artifact, record its byte count and SHA-256, and restart fresh ordered specification review, code-quality review, and independent verification against that exact artifact. Any edit or finding invalidates the artifact and all verdicts and requires rerunning every GREEN gate before the existing H08 commit step.
...
#### H09 execution-plan hardening

The original unit sketch is not compatible with the current private helpers and would also change the persisted CAMT marker identity: `bankReferenceHash()`, `bankReferenceLookupKey()`, and `buildCamtEntrySignature()` currently use the trimmed, case-sensitive bank reference, and trusted `br`/`brh`/`sig` markers already exist with that exact unscoped representation. The defect is later in the data flow. H08 proves `accounts_dimensions_id` before import, but `import_camt053` drops it when calling `enrichWithDuplicates`; `buildDuplicateLookup`, `buildExistingTransactionDuplicateKey`, `buildExistingDuplicateKeyForEntry`, and the broad `byBankRef` map consequently compare globally. This suppresses a legitimate row when another own-bank dimension has the same direct reference, and the trusted-description/hash exact-row fallback has the same cross-dimension defect. Execute this correction plan instead of the original Steps 1-4; keep the original H09 commit message.

1. **Commit the approved plan, freeze scope, and record the implementation base before adding tests.** H08 code remains the measured behavioral baseline at `4cd82a5`. After this hardening section is approved, commit only `docs/superpowers/plans/2026-07-15-full-codebase-remediation.md` with `git commit -m "docs(H09): harden CAMT dimension-scoped duplicate plan"`; record the resulting commit hash as `H09_IMPLEMENTATION_BASE`, and require `git status --short` to be empty before any H09 source/test edit. All implementation diffs, artifacts, reviews, and scope checks use that plan commit—not dirty `4cd82a5`—as their base, while recording `4cd82a5` separately as the H08 code baseline. Work only in `src/tools/camt-import.ts` and `src/tools/camt-import-tools.test.ts`; do not export private duplicate helpers, edit parser-only `src/tools/camt-import.test.ts`, change CAMT XML parsing, alter H08 account binding, or start H10. Record these measured baseline results, which the plan-only commit does not change: `npx vitest run src/tools/camt-import-tools.test.ts -t "H08"` is 12 PASS/24 skipped; `npx vitest run src/tools/camt-import.test.ts src/tools/camt-import-tools.test.ts` is 42/42; `npm test` is 2,487/2,487 across 76 files; `npm run build` and `npm run validate:release` pass; and `npm run test:integration` is 20 PASS/3 documented skips.

2. **Write the H09 handler-level regressions first, with no production edit.** Add ten tests and H09-tag one corrected existing compatibility test in `src/tools/camt-import-tools.test.ts`. Use statement dimension `7`, other valid dimension `8`, the default statement identity `EE637700771011212909`, and the exact names/data below. Each import assertion must cover the public payload (`mode`, `created_count`, `skipped_count`, `summary`, `sample`, and `execution.results`/`execution.skipped`) and the real `transactions.create` spy appropriate to dry-run or execute; do not test newly exported internals.

   1. `H09 imports an equal direct bank reference owned by another bank dimension`: use the single-entry `REF-VOID-1` statement and existing transaction `{ id: 901, accounts_dimensions_id: 8, status: "PROJECT", bank_ref_number: "REF-VOID-1", date: "2026-01-31", type: "C", amount: 999, cl_currencies_id: "EUR" }`; execute on dimension 7 must create one, skip zero, and never report ID 901.
   2. `H09 still skips an equal direct bank reference on the selected bank dimension`: use the same deliberately different date/amount transaction with dimension 7; execute must create zero, skip one, and report `[901]`. This is a pre-existing-behavior negative control.
   3. `H09 ignores reference candidates with missing or malformed transaction dimensions`: in one table inside the test, run independent execute cases for `undefined`, `null`, `"7"`, `0`, `-7`, `7.5`, and `Number.NaN` injected through `unknown`; every same-reference candidate must be ignored, create one, skip zero, and never leak an invalid runtime value into output. A numeric string is not silently coerced and no own dimension is inferred from marker metadata, counterparty IBAN, or the H08 statement binding.
   4. `H09 preserves case-sensitive bank reference identity within one dimension`: existing dimension-7 reference `ref-void-1` must not suppress statement reference `REF-VOID-1`; assert one create and zero skips. This is a PASS control for the current trimmed, case-sensitive hash semantics; do not uppercase or otherwise case-fold reference content.
   5. `H09 accepts legacy trusted reference markers on the selected bank dimension`: table within this single test over both persisted pre-H09 variants: (a) short reference `REF-VOID-1` with exact marker `Test payment\n[e-arveldaja-mcp:camt br=REF-VOID-1 sig=829b819216e09dd3]`; and (b) long reference `REF-` plus `"1234567890".repeat(20)` with exact marker `Test payment\n[e-arveldaja-mcp:camt brh=sha256:6dad99eb61334998e434cf2e593253b6c357e519d7d352e14e4beaf7c19a6f89 sig=699a9250bc1ba018]`. For each independent setup, transaction 902 has dimension 7, null direct `bank_ref_number`, date `2026-02-01`, type `C`, currency `EUR`, amount `10`, name `Vendor OÜ`, and no ref number/account number; feed a statement with the corresponding reference and assert skip `[902]`. This remains one legacy-marker PASS control and proves both trusted `br` and `brh` fallbacks retain their current unscoped signature bytes.
   6. `H09 does not accept legacy trusted reference markers from another bank dimension`: table within this single test over the same exact short `br=REF-VOID-1 sig=829b819216e09dd3` and long `brh=sha256:6dad99eb61334998e434cf2e593253b6c357e519d7d352e14e4beaf7c19a6f89 sig=699a9250bc1ba018` marker cases, matching row fields, and corresponding statement references, but set transaction 902 to dimension 8. Each independent setup must create one, skip zero, and omit ID 902. This remains one RED test and proves both stored metadata variants are scoped only after their existing signature is trusted.
   7. `H09 parse_camt053 makes no unscoped persisted-duplicate claim`: call granular `parse_camt053` for `REF-VOID-1` with existing same-reference transaction 903 on dimension 8. Table inside this one test over (a) `bankAccounts: []` and (b) ambiguous bank-account rows that bind the statement identity to both dimensions 7 and 8. For each independent setup, require `transactions.listAll`, `readonly.getAccountDimensions`, and `readonly.getBankAccounts` all not called; require `summary.duplicate_count === 0`, the entry's normal parsed fields unchanged, and serialized `duplicate`/`duplicate_transaction_ids` absent. Parse-only must neither guess a dimension nor fail for absent/ambiguous binding.
   8. `H09 process_camt053 parse preserves its wrapper without an unscoped duplicate claim`: use the same candidate through `process_camt053` mode `parse`, again tabled inside this one test over absent bank accounts and the same cross-dimension ambiguous binding. Retain `recommended_entry_point`, `mode`, `delegated_tool`, `delegated_args`, wrapped external entry text, entry/credit/debit counts, and nested result shape, while requiring zero duplicate count, no duplicate ID fields, and `transactions.listAll`, `readonly.getAccountDimensions`, and `readonly.getBankAccounts` all not called in every setup.
   9. `H09 process_camt053 dry-run scopes duplicate lookup to the selected bank dimension`: with transaction 904 on dimension 8 and the same reference, mode `dry_run` for dimension 7 must retain the exact wrapper/delegation/approval-envelope contract and nested `DRY_RUN` payload, return one `would_create`, skip zero, and perform no create.
   10. `H09 imports both repeated-reference rows when the only exact row is on another dimension`: reuse the two-row `REF-SPLIT-1` statement from the current split-row tests and an otherwise exact Vendor A/E2E-1 transaction 905 on dimension 8; require both rows eligible in statement order, zero skips, and no 905. The repeated-reference guard must continue disabling the broad reference fallback while the row-exact lookup is dimension-scoped.

   Rename the existing `only skips the already imported split row when a prior import used the same shared bank reference` test to `H09 only skips the already imported repeated-reference row on the selected bank dimension`, add `accounts_dimensions_id: 7` to transaction 501, and retain its exact one-created/one-skipped `[501]` assertions. This fixture correction documents that rows with missing ownership are no longer valid exact-reference candidates; it does not change the test count.

3. **Prove RED and preserve the controls before implementation.** Run `npx vitest run src/tools/camt-import-tools.test.ts -t "H09"`; expected current-production result is exactly **7 FAIL / 4 PASS**. The PASS controls are same-dimension direct reference, case-sensitive inequality, the one same-dimension legacy-marker test covering both trusted `br` and `brh` variants, and the corrected same-dimension repeated-reference test. The seven failures are the direct cross-dimension case, malformed/missing-dimension table, the one other-dimension legacy-marker test covering both variants, granular parse-only, merged parse-only, merged dry-run, and repeated-reference cross-dimension case; each must fail only because current global lookup marks/skips a duplicate or reads transactions in parse-only. Then run `npx vitest run src/tools/camt-import.test.ts src/tools/camt-import-tools.test.ts`; expected **7 FAIL / 45 PASS out of 52**. Fixture, XML parse, H08 binding, wrapper-shape, mock-reset, or unexpected-error failures are invalid RED evidence and must be corrected in tests before production work.

4. **Add a dimension-scoped lookup identity without changing persisted marker identity.** Keep `normalizeOptionalReference`, `bankReferenceHash(value)`, `bankReferenceLookupKey(value)`, `buildCamtEntrySignature`, marker emission, `extractCamtDescriptionMetadata`, `normalizeStoredBankReferenceHash`, `normalizeStoredEntrySignature`, and `isTrustedCamtDescriptionMetadata` byte-compatible and case-sensitive. Add one private helper beside `bankReferenceLookupKey`, with this contract:
   ...
   function dimensionScopedBankReferenceLookupKey(
     ...
     return unscopedReferenceKey && typeof dimensionId === "number" &&
       Number.isSafeInteger(dimensionId) && dimensionId > 0
       ? `${dimensionId}\0${unscopedReferenceKey}`
       : undefined;
   }
   ```

   The helper consumes the already-generated unscoped case-sensitive hash; it does not hash `dimensionId + reference`, change `brh`, include dimension in `sig`, uppercase a reference, or persist a new marker field. A missing, null, non-number, non-finite, unsafe, fractional, zero, or negative transaction dimension produces no lookup key. This is fail-open for import eligibility, not a validation error: malformed historical rows cannot prove duplicate ownership and remain available to the separate dimension-scoped possible-duplicate review only if they satisfy that existing subsystem's rules.

5. **Thread the H08-selected dimension through exact-reference enrichment only.** Make `enrichWithDuplicates(parsed, api, selectedDimensionId: number)` require the validated dimension; call it from `import_camt053` as `enrichWithDuplicates(loaded, api, accounts_dimensions_id)` after the existing H08 guard. Make `buildDuplicateLookup(transactions, selectedDimensionId)`, `buildExistingTransactionDuplicateKey(transaction, selectedDimensionId)`, `buildExistingDuplicateKeyForEntry(entry, selectedDimensionId)`, and `findDuplicateTransactionIds(entry, lookup, repeatedBankReferences, selectedDimensionId)` use `dimensionScopedBankReferenceLookupKey` for both `byEntryKey` and `byBankRef`. In `buildDuplicateLookup`, reject a transaction before direct-reference or trusted-metadata/hash extraction unless its runtime `accounts_dimensions_id` is a positive safe integer equal to the selected dimension. For valid same-dimension rows, keep direct `bank_ref_number` broad matching unchanged; keep trusted `br`/`brh` fallback row-exact only, with the existing signature verification and exact stored hash bytes; never promote unsigned/invalid marker metadata into broad reference matching. Preserve ID deduplication/sort order, VOID filtering, the second existing-transaction read used by possible-duplicate review, date filtering, batch keys, and all create/client/progress/audit behavior.

6. **Make dimensionless parse-only behavior explicit and safe.** `parse_camt053` and `process_camt053` mode `parse` must continue parsing without `accounts_dimensions_id`, H08 bank-account lookup, or mutation. Since they cannot prove which own-bank dimension owns the statement, do not call `enrichWithDuplicates`, `transactions.listAll`, `readonly.getAccountDimensions`, or `readonly.getBankAccounts`; return the parser's `duplicate: false`, empty IDs, and zero duplicate count, with the current serializer continuing to omit false/empty duplicate fields. Adjust only the granular tool description as needed so it promises parsed statement preview rather than persisted duplicate matches. Do not add an optional dimension argument, silently resolve by IBAN, or reuse H08 bank-account ownership as an implicit selector. Dry-run and execute still perform the H08 guard first and then dimension-scoped enrichment. Preserve `process_camt053` delegation fields, approval remapping, public result/wrapper shape, untrusted-text wrapping, summaries, result ordering, reason strings, and `batch_execution_v1` contracts.

7. **Prove GREEN and all repository gates.** Run in order:

   ```bash
   npx vitest run src/tools/camt-import-tools.test.ts -t "H09"
   npx vitest run src/tools/camt-import.test.ts src/tools/camt-import-tools.test.ts
   npm run build
   npm run validate:release
   git diff --check
   npm test
   ...
   Require H09 **11/11**, both CAMT files **52/52**, full unit **2,497/2,497** across 76 files, build/release/diff checks PASS, and integration **20 PASS/3 documented skips**. Require exactly `src/tools/camt-import.ts` and `src/tools/camt-import-tools.test.ts` in the implementation diff. Recheck specifically: valid same-dimension direct and legacy-marker duplicates still skip; case-only reference differences do not collide; another/missing/malformed dimension never suppresses; repeated references still use row-exact matching within the selected dimension; parse-only stays binding-free and makes no unscoped duplicate claim; dry-run/execute/wrapper/output contracts are unchanged apart from corrected duplicate classification. No H10 file, EUR-transfer behavior, parser behavior, marker format, hash/signature bytes, or public input schema may change.

8. **Freeze the artifact against the committed plan base, obtain independent reviews, record the ledger, and commit only after approval.** Generate `.omc/reviews/H09.diff` as the exact binary-safe diff of only `src/tools/camt-import.ts` and `src/tools/camt-import-tools.test.ts` against the recorded `H09_IMPLEMENTATION_BASE`; do not diff against `4cd82a5`, because that would mix this approved plan edit into the implementation artifact. Record byte count and SHA-256, and prove the frozen artifact byte-matches the live two-file diff from `H09_IMPLEMENTATION_BASE` before each review. First give a fresh specification reviewer only the H09 finding, H08 code baseline `4cd82a5`, committed-plan implementation base, this hardening section, and frozen artifact; require `SPEC COMPLIANCE: APPROVED` for cross-dimension direct/exact isolation, malformed ownership, dimensionless parse policy, both legacy `br`/`brh` marker/hash/signature variants, case semantics, repeated references, wrapper/output contracts, tests, and exact scope. Only after that approval, give a different fresh code-quality reviewer the same immutable artifact and require `CODE QUALITY APPROVED` for validation-before-metadata use, one scoped-key construction path, type precision, no accidental case-folding or persisted-hash drift, no global fallback, deterministic IDs/order, and test strength. Then obtain independent verification of the recorded gates. Any edit or finding invalidates the artifact and all verdicts; add/correct the failing test first, reproduce RED, make the smallest H09 fix, rerun Step 7, regenerate the artifact from `H09_IMPLEMENTATION_BASE`, and restart reviews in order.

   Append one H09 row to ignored `.omc/full-codebase-remediation-ledger.md` with H08 code baseline `4cd82a5`, the recorded committed-plan `H09_IMPLEMENTATION_BASE`, measured baseline, exact 7 RED/4 control names, 11/11 and 52/52 GREEN, full 2,497/2,497 and integration evidence, direct/exact/both stored-marker variants/malformed/case/repeated/parse-wrapper zero-read contract evidence, artifact bytes/SHA-256/live identity, ordered reviewer verdicts, exact two-file scope, and final commit hash. Then run the original Task 21 commit step with `git add src/tools/camt-import.ts src/tools/camt-import-tools.test.ts` and `git commit -m "fix(H09): scope CAMT references by bank account"`. Because the plan was committed before implementation and `.omc/reviews/H09.diff` plus `.omc/full-codebase-remediation-ledger.md` are ignored artifacts, require `git status --short` to be empty after the implementation commit and H09 to be at `HEAD`. Do not begin H10.
...
#### H10 execution-plan hardening

The original sketch identifies the visible symptom but does not close the monetary-provenance boundary. In Phase 2, `comparableTransactionAmount(tx)` currently supplies `roundMoney(tx.base_amount ?? tx.amount)` to `BookingGuard.resolveInterAccount()` and `recordInterAccountJournal()`, while confirmation, dry-run/execute results, and audit text/details independently use raw `tx.amount` and label it EUR. A one-sided transaction with `amount: 100`, `base_amount: 90`, and currency USD can therefore be looked up and recorded under EUR 90 but posted and reported as EUR 100. A foreign transaction with only `currency_rate` is looked up, posted, and reported as the nominal amount because the current comparator has no rate fallback. Missing/malformed runtime values can throw during rounding or silently pass into matching, and the original proposed helper would still default missing currency to EUR, trust contradictory evidence, ignore invalid lower-precedence fields, and leave paired-transfer scope ambiguous. Execute this correction plan instead of the original Steps 1-4; keep the original H10 implementation commit message.

1. **Commit the approved plan as the clean implementation base before adding tests.** The measured H09 code baseline is `ff0c20c`. After this hardening section is approved, commit only `docs/superpowers/plans/2026-07-15-full-codebase-remediation.md` with `git commit -m "docs(H10): harden one-sided FX amount plan"`; record the resulting hash as `H10_IMPLEMENTATION_BASE`, and require `git status --short` to be empty before any H10 source/test edit. All H10 implementation diffs, artifacts, reviews, and scope checks use that plan commit as their base while retaining `ff0c20c` as the code baseline. Work only in `src/tools/bank-reconciliation.ts` and `src/tools/bank-reconciliation.test.ts`. Do not export the new private helper, change `Transaction` API types, alter Phase 1 C/D or Phase 1b reciprocal-pair matching/posting/review behavior, edit `BookingGuard`, or begin Task 23 M03, Task 24 M04, or Task 25 M05. The measured baseline at `ff0c20c` is: `npx vitest run src/tools/bank-reconciliation.test.ts` **71/71**; `npm test` **2,497/2,497 across 76 files**; `npm run build` and `npm run validate:release` PASS; and `npm run test:integration` **20 PASS/3 documented skips**.

2. **Write exactly twelve handler-level H10 tests before production edits.** Add a hoisted `logAudit` spy in `src/tools/bank-reconciliation.test.ts` so audit amount assertions do not inspect or write the filesystem, clear it between cases, and update every existing Phase-2 fixture that intentionally represents an EUR transaction to carry explicit `cl_currencies_id: "EUR"`; do not make `setupInterAccountTool` synthesize currency, base amount, or rate. Add these exact H10-tagged cases, keeping each described table inside one `it` so the selected count is exactly twelve:

   1. `H10 keeps foreign-base C one-sided dry-run and execute amounts identical`: use `type: "C", amount: 100, base_amount: 90, cl_currencies_id: " usd "`, source Wise dimension 300, and target LHV dimension 100. Independent granular dry-run, execute, existing-EUR-90-journal, and merged `inter_account_dry_run` setups must all report source 300/Wise and target 100/LHV in the one-sided or already-handled path. Granular and nested merged dry-run rows retain nominal `amount: 100`, normalized `currency: "USD"`, and `amount_eur: 90`, with the granular row duplicated in `execution.results`; execute confirms exactly `{ related_sub_id: 100, amount: 90 }`, returns the same mapping/value, and audits 90 in summary/details. The existing-journal setup must skip on EUR 90 and expose the same nominal/currency/EUR fields in both `already_handled` and `execution.skipped`. This is the successful foreign merged-wrapper parity proof; dry-run/merged perform no get/update/confirm/audit.
   2. `H10 keeps foreign-base D one-sided dry-run and execute amounts identical`: use `type: "D"`, source LHV dimension 100, target Wise dimension 300, and the same nominal/base/currency evidence. Independent granular dry-run, execute, and existing-journal setups must report source 100/LHV and target 300/Wise in public and execution arrays; execute confirms exactly `{ related_sub_id: 300, amount: 90 }`, while the guard outcome skips against EUR 90. Preserve the existing D direction/dimension semantics rather than reversing labels as part of H10.
   3. `H10 derives rate-only foreign EUR amounts in both one-sided directions`: table inside one test over C (source 300 -> target 100) and D (source 100 -> target 300) with `amount: 100`, no base, `currency_rate: 0.9`, and USD. For each direction, use independent dry-run, execute, and pre-existing-EUR-90-journal fixtures; require the exact source/target mapping in granular `one_sided`/`already_handled`, `execution.results`/`execution.skipped`, guard outcome, and confirmation distribution. Every surface and audit uses EUR 90, never nominal 100.
   4. `H10 prefers agreeing foreign base evidence over rate evidence`: use two ref-less same-date/source/target one-sided rows, each `amount: 100`, `base_amount: 90`, `currency_rate: 0.8999`, and USD. The rounded rate evidence is EUR 89.99, exactly one cent below base, so base EUR 90 remains authoritative. Execute must confirm only the first row at 90 and record that same key; the second must resolve as `ambiguous_refless`, with nominal 100, currency USD, and `amount_eur: 90` in granular `one_sided`/`ambiguous_refless` and the corresponding `execution.results`/`execution.skipped` entries. This concrete first-confirm/second-collision sequence proves the in-run guard record uses the same amount as posting/output. Also include an independent exact-agreement control with rate `0.90004`, which rounds to 90 and retains base precedence.
   5. `H10 rejects contradictory base and rate before consuming BookingGuard state`: use two same-date/same-dimension one-sided rows plus one pre-existing ref-less EUR-90 journal. The first row is USD 100/base 90/rate `0.8998`, whose rounded rate evidence is EUR 89.98 (a two-cent conflict), and must return `one_sided_eur_amount_conflict`; the second is valid USD 100/base 90 and must still be reported in `already_handled` against that journal. Require `already_handled` and its matching `execution.skipped` entry to contain nominal `amount: 100`, `currency: "USD"`, and `amount_eur: 90`, with zero transaction get/update/confirm calls. Together with case 4, this proves integer-cent policy: a one-cent difference is accepted and a two-cent difference is rejected before guard consumption.
   6. `H10 rejects an EUR nominal/base conflict before client or confirmation mutation`: EUR 100/base 90 must return `one_sided_eur_amount_conflict`, produce no one-sided match, and perform no transaction get/update/confirm or audit call.
   7. `H10 rejects malformed one-sided numeric fields with stable precedence`: build every row with an independent handler/API fixture, invoke each inside its own `try`/`catch`, and collect every returned payload or thrown error before asserting the table so one current `roundMoney` throw cannot abort the remaining rows. Cover invalid nominal amount (`undefined`, `null`, numeric string, zero, negative, `NaN`, `Infinity`), invalid present base, invalid present rate, multiplication overflow (`amount: Number.MAX_VALUE`, foreign rate `2`, no base), and multiplication underflow (`amount: Number.MIN_VALUE`, foreign rate `0.5`, no base). Overflow/underflow must resolve to `one_sided_currency_rate_invalid`, not throw or become missing/conflict. Include stacked-invalid rows proving exact nominal -> currency -> base -> rate precedence: all four invalid selects nominal; valid nominal plus invalid currency/base/rate selects currency; valid nominal/currency plus invalid base/rate selects base; valid nominal/currency with absent base plus invalid rate selects rate. For each row require the exact code and literal reason, require the reason itself not to contain `String(rawNumericValue)`, and prove zero transaction get/update/confirm/audit calls. A pre-fix `roundMoney` throw captured from cases intended to exercise malformed numeric behavior is valid load-bearing RED evidence; only fixture/import/audit-spy failures are invalid RED.
   8. `H10 rejects missing currency and missing foreign EUR evidence without mutation`: table inside one test over missing/null/blank/non-string, non-ASCII, and shape-invalid currency plus foreign rows with both base and rate absent/null. Currency acceptance is shape-only ASCII `^[A-Z]{3}$` after trim/uppercase because this repository has no authoritative currency allowlist; do not call syntactically valid but unknown three-letter codes “non-ISO.” Require exact stable codes/messages, zero matches, and zero transaction get/update/confirm/audit calls. Null/undefined optional base or rate means absent; a present invalid value is never ignored merely because higher-precedence evidence exists.
   9. `H10 never exposes malformed currency through granular or merged output`: use an instruction-like currency string in both `reconcile_inter_account_transfers` dry-run and `reconcile_bank_transactions` `inter_account_dry_run`; require `one_sided_currency_invalid`, the fixed safe message, intact merged wrapper/delegation/execution shape, and absence of the raw string from both serialized responses.
   10. `H10 control preserves EUR C and D one-sided nominal posting`: table inside one test over C and D, explicit EUR 75 and no base/rate; require the existing target dimensions, public nominal amount, and confirmation distribution 75. This is an intended pre-change PASS control and does not assert the new EUR field during RED.
   11. `H10 control preserves paired EUR single-confirm behavior`: retain one normal C/D EUR pair and prove one confirm, one duplicate delete, existing pair result fields/order, and no Phase-2 result. This is an intended pre-change PASS control.
   12. `H10 control preserves paired FX manual review behavior`: retain the existing base-only cross-currency pair shape and prove `needs_review_cross_currency: 1`, zero confirm/delete, and no Phase-2 result. This is an intended pre-change PASS control.

   Run `npx vitest run src/tools/bank-reconciliation.test.ts -t "H10"`; expected test-only RED is exactly **9 FAIL/3 PASS out of 12**. The three PASS controls are cases 10-12. Then run the whole file; expected RED is **9 FAIL/74 PASS out of 83**. A fixture/import failure, audit-spy setup failure, paired-control regression, or failure outside the nine named behavior cases is invalid RED and must be corrected in tests before production work. Case 7's independently caught current-production `roundMoney` throws are intended behavior failures and therefore valid RED, not setup failures.

3. **Define one private, runtime-safe authoritative resolver with explicit precedence.** Add private `OneSidedEurAmountErrorCode`, fixed message constants, and a discriminated-union result beside the existing amount helpers. `resolveOneSidedTransferAmount(tx)` must inspect the runtime values as `unknown`, never throw for malformed upstream data, and return either `{ ok: true, nominalAmount, currency, amountEur }` or `{ ok: false, code, reason }`. Use exactly these code/message literals without interpolating raw values:
   ...
   Validate in exact nominal -> currency -> base -> rate -> missing evidence -> conflict order. Normalize currency with `trim().toUpperCase()` and accept only shape-valid ASCII `^[A-Z]{3}$`; this is deliberately not an ISO-membership claim because no authoritative allowlist exists in the repository. Treat only `undefined`/`null` as absent optional base/rate; validate every present field even when higher-precedence evidence is usable. When rate evidence is needed or checked, classify a non-finite, zero, or negative `amount * currency_rate` product—including numeric overflow to `Infinity` and underflow to zero—as `one_sided_currency_rate_invalid` before calling `roundMoney`.

   For normalized currency `EUR`, nominal `roundMoney(amount)` is authoritative; a present base must agree with it in integer cents, while a valid present rate is compatibility metadata and does not replace the EUR nominal amount. For every non-EUR currency, precedence is (1) rounded positive `base_amount`, then (2) rounded `amount * currency_rate`; if both exist, compute integer cents for each rounded value and reject only when `Math.abs(baseCents - rateCents) > 1`, otherwise retain the rounded base as authoritative. Apply the same integer-cent comparison to EUR nominal/base evidence: exactly one cent is accepted, two cents is rejected. Never compare conflicts with raw floating subtraction, default missing currency to EUR, use foreign nominal amount as EUR evidence, or silently accept contradictory/malformed lower-precedence evidence. Keep `amount` as the public nominal field, add normalized `currency`, and add the unambiguous `amount_eur`; do not rename or reinterpret `amount` as EUR.

4. **Apply the resolver only at the eligible Phase-2 boundary and thread one value everywhere.** After `inferOneSidedTransfer(tx)` establishes an eligible target but before `resolveExistingJournal(..., consume: true)`, `consumedTxIds.add`, `ensureClientsId`, or any transaction mutation, call the resolver exactly once. On failure, append `{ transaction_ids: [tx.id], code, reason }` to `errors` and continue. On success, bind `amountEur` and use that identical value for the BookingGuard lookup, `recordInterAccountJournal`, `buildAccountDistribution`, dry-run `one_sided[].amount_eur`, execute `one_sided[].amount_eur`, `execution.results`, and audit summary/details. Preserve nominal `amount`, normalized `currency`, type, dimensions, confidence, reasons, wrapped description, and wrapped counterparty name. For one-sided `already_handled` and `ambiguous_refless` output, retain nominal `amount` and add normalized `currency` plus `amount_eur` so the amount used for journal resolution is not silently mislabeled; cases 4 and 5 must prove those fields also survive in `execution.skipped`. Audit summary must interpolate `amountEur` as EUR; details must contain `{ amount: nominalAmount, currency, amount_eur: amountEur, date }`. Errors expose only exact code/reason literals. For numeric invalidity, assert non-echo on the `reason` field rather than the entire response because IDs/counts may legitimately contain the same digits; whole-response non-echo is required only for case 9's deliberately unique unsafe currency strings. Do not pass raw currency into an error, reason, audit, or wrapper summary.

   Do not replace or broaden `comparableTransactionAmount()` for Phase 1/1b: paired matching, paired duplicate lookup/recording, cross-currency review, pair result shape, single-confirm/delete policy, and paired audit behavior stay byte-compatible except for fixture-only explicit EUR additions where necessary. Do not add inter-account execute mode to the merged wrapper. The granular dry-run/execute paths and merged dry-run must share the same Phase-2 resolver and result data; there must be no second conversion in preview, posting, journal indexing, result construction, or audit.

5. **Prove GREEN and every repository gate in order.** Run:

   ```bash
   npx vitest run src/tools/bank-reconciliation.test.ts -t "H10"
   npx vitest run src/tools/bank-reconciliation.test.ts
   npm run build
   npm run validate:release
   git diff --check
   npm test
   ...
   Require H10 **12/12**, the bank-reconciliation file **83/83**, full unit **2,509/2,509 across 76 files**, build/release/diff checks PASS, and integration **20 PASS/3 documented skips**. Require exactly `src/tools/bank-reconciliation.ts` and `src/tools/bank-reconciliation.test.ts` in the implementation diff. Recheck explicitly: base-first and rate-only foreign handling; agreeing and contradictory dual evidence; EUR nominal/base conflict; missing/null/malformed/overflow values; both C and D one-sided directions; validation before guard consumption and client mutation; dry-run/execute/merged parity; identical EUR amount in lookup, record, distribution, output, execution contract, and audit; nominal/currency compatibility; stable non-echoing error output; and unchanged EUR/FX paired behavior. No M03/M04/M05 file or behavior may change.

6. **Freeze the exact artifact, obtain ordered independent reviews, record the ledger, then commit.** Generate ignored `.omc/reviews/H10.diff` as the binary-safe diff of exactly the two scoped files against `H10_IMPLEMENTATION_BASE`; record byte count and SHA-256 and prove it byte-matches the live two-file diff before each review. First give a fresh specification reviewer only the H10 finding, baseline `ff0c20c`, committed-plan base, this hardening section, and frozen artifact; require `SPEC COMPLIANCE: APPROVED` for precedence/validation, C/D coverage, guard-before-mutation ordering, dry/execute parity, output/audit consistency, safe errors, paired compatibility, tests, and exact scope. Only after that approval, give a different fresh code-quality reviewer the same immutable artifact; require `CODE QUALITY APPROVED` for one resolver/one conversion, runtime validation before rounding, deterministic error precedence, no raw unsafe interpolation, no duplicated monetary derivation, and no paired-flow drift. Then obtain independent verification of all recorded gates. Any edit or finding invalidates the artifact and all verdicts: add or correct the failing H10 test first, reproduce RED, make the smallest Phase-2 fix, rerun every gate, regenerate the artifact, and restart reviews in order.

   Append one H10 row to ignored `.omc/full-codebase-remediation-ledger.md` with code baseline `ff0c20c`, `H10_IMPLEMENTATION_BASE`, measured baseline, exact nine RED and three control names/counts, 12/12 and 83/83 GREEN, full 2,509/2,509 and integration evidence, precedence/validation/C-D/guard-order/parity/safe-output/paired-control evidence, artifact bytes/SHA-256/live identity, ordered reviewer verdicts, exact two-file scope, and final commit hash. Then run the original Task 22 commit step with `git add src/tools/bank-reconciliation.ts src/tools/bank-reconciliation.test.ts` and `git commit -m "fix(H10): post one-sided transfers in base EUR"`. Because the plan was committed before implementation and the artifact/ledger are ignored, require `git status --short` to be empty after the implementation commit and H10 to be at `HEAD`. Do not begin M03, M04, or M05.

#### H10 first review-correction plan

The frozen 51,129-byte H10 artifact whose recorded SHA-256 begins `914e` failed specification review on overflow safety. Its integer-cent conflict comparison multiplies already-rounded huge finite values by 100; both products can become `Infinity`, so `Infinity - Infinity` becomes `NaN` and the `> 1` conflict test returns false. This can accept materially different EUR evidence as agreeing. Invalidate that artifact and all verdicts. Preserve the twelve-test H10 suite and exact file scope; apply this correction TDD-first before regenerating or reviewing any artifact.

1. **Strengthen existing H10 case 5 with two independently invoked huge-value rows before production edits.** Do not add a thirteenth H10 test and do not modify case 6. Inside existing case 5, create two fully independent handler/API setups and invoke both with `{ execute: true }`, collecting both parsed payloads before asserting either so one failure cannot mask the other. The foreign setup uses `amount: 1e308`, `base_amount: 1e308`, `currency_rate: 0.5`, and a shape-valid foreign currency; give that setup its own matching registered ref-less journal with two own-bank postings of `1e308` on the transaction date/dimensions. The EUR setup uses `amount: 1e308`, `base_amount: 5e307`, and `cl_currencies_id: "EUR"`; give it a separately constructed matching registered ref-less journal with two own-bank postings of `1e308` on its date/dimensions. Under the current flawed cent-scaling comparison, each conflict is incorrectly accepted, reaches `resolveExistingJournal`, and produces one `already_handled` result; assert that current behavior only as the load-bearing reason the new expectations fail, not as desired behavior. Corrected behavior must return code `one_sided_eur_amount_conflict` and exact reason `The one-sided transfer EUR amount evidence conflicts by more than one cent.` for both payloads before lookup/consume, with `already_handled`, `ambiguous_refless`, and `one_sided` all empty and transaction get/update/confirm plus audit all called zero times in each setup. Keep case 5's ordinary exact-two-cent rejection row and case 4's exact-one-cent acceptance row unchanged.

   Run `npx vitest run src/tools/bank-reconciliation.test.ts -t "H10"`; expected correction RED is exactly **1 FAIL/11 PASS out of 12**, with only strengthened case 5 failing because both current huge-value paths produce `already_handled` after non-finite cent-comparison intermediates. Run `npx vitest run src/tools/bank-reconciliation.test.ts`; expected **1 FAIL/82 PASS out of 83**. If the full unit suite is run at this test-only checkpoint, expected is **1 FAIL/2,508 PASS out of 2,509**. A thrown rounding error, fixture/import failure, unexpected mutation, wrong journal fixture/key, wrong code/reason, modification or failure of case 6, or any additional failing H10 case is invalid correction RED and must be fixed in the test setup before production work.

2. **Replace raw cent scaling with one private overflow-safe comparison helper used by both currency paths.** Add exactly one private `amountsDifferByMoreThanOneCent(leftRounded: number, rightRounded: number): boolean` beside the H10 resolver and call it for both EUR nominal/base and foreign base/rate comparisons. Inputs are already validated finite positive and rounded with `roundMoney`. When both amounts can be converted to safe integer cents without overflow or precision loss, compare those integer cents and return `Math.abs(leftCents - rightCents) > 1`, preserving exact-one-cent acceptance and two-cent rejection. Otherwise, do not multiply by 100 and do not use raw floating subtraction against `0.01`: at magnitudes where a finite rounded value cannot be represented as safe integer cents, IEEE-754 spacing already exceeds one cent, so return false only when `Object.is(leftRounded, rightRounded)` (or strict numeric equality) and return true for any distinct finite rounded values. The helper must never produce or compare `Infinity`/`NaN`, silently treat a non-finite intermediate as agreement, or introduce a second EUR-evidence comparison path. Leave validation precedence, base-first authority, rate-only derivation, guard ordering, output/audit fields, C/D direction mappings, merged parity, stable errors, paired behavior, and M03/M04/M05 scope unchanged.

3. **Rerun all GREEN gates, freeze a new artifact, and restart ordered review.** Run the existing H10 gate sequence and require H10 **12/12**, bank reconciliation **83/83**, full unit **2,509/2,509 across 76 files**, build/release/diff checks PASS, and integration **20 PASS/3 documented skips**. Require the implementation diff to remain exactly `src/tools/bank-reconciliation.ts` and `src/tools/bank-reconciliation.test.ts`. Regenerate `.omc/reviews/H10.diff` from `H10_IMPLEMENTATION_BASE`, record its new byte count/SHA-256, and prove frozen/live identity. Restart specification review first, then code-quality review, then independent verification against the same immutable replacement artifact; the failed 51,129-byte artifact and every prior verdict remain invalid. Update the H10 ledger row with the correction RED counts, both huge-value cases, overflow-safe helper evidence, final GREEN gates, replacement artifact identity, fresh ordered verdicts, and only then use the original H10 implementation commit step. Any further edit or finding again invalidates the artifact and all reviews. Do not implement or begin M03, M04, or M05 as part of this correction.

#### H10 second review-correction plan

Before freezing the replacement artifact, coordinator review found a second defect in the proposed overflow-safe helper: using `Number.isSafeInteger(value * 100)` as the gate rejects ordinary exact-cent money because binary multiplication is not necessarily an integer (`0.07 * 100` can be `7.000000000000001`). The helper then falls into huge-magnitude equality-only comparison and incorrectly treats a permitted one-cent difference as a conflict. Invalidate the current unfrozen implementation checkpoint and any verdict derived from it; keep the prior huge-value correction, twelve-test count, and exact two-file scope.

1. **Strengthen existing H10 case 4 with independent small-money boundary setups before changing production again.** Do not add a new H10 test. Inside case 4, add two independent foreign one-sided handler/API setups and collect both payloads before assertions. The acceptance setup uses `amount: 1`, `base_amount: 0.07`, and `currency_rate: 0.06`; the evidence differs by exactly one cent, so execute must accept base precedence, confirm/distribute exactly EUR `0.07`, return nominal `amount: 1`, normalized currency, and `amount_eur: 0.07` in `one_sided` plus `execution.results`, record the guard at `0.07`, and audit `0.07`. The paired rejection setup uses `amount: 1`, `base_amount: 0.07`, and `currency_rate: 0.05`; the two-cent difference must return `one_sided_eur_amount_conflict` with the exact fixed reason before guard/client/confirm/audit work and with empty `already_handled`, `ambiguous_refless`, and `one_sided`. Preserve in the same case the existing ordinary EUR 90 versus 89.99 one-cent acceptance, the EUR 90 versus 89.98 two-cent rejection in case 5, and both huge-value regressions from the first correction plan.

   Run `npx vitest run src/tools/bank-reconciliation.test.ts -t "H10"`; expected correction RED is exactly **1 FAIL/11 PASS out of 12**, with only strengthened case 4 failing because the current `Number.isSafeInteger(value * 100)` gate sends `0.07`/`0.06` to equality-only comparison. Run the whole bank-reconciliation file and require correction RED **1 FAIL/82 PASS out of 83**. Any additional failure, change to the huge-value results, acceptance of the `0.07`/`0.05` pair, mutation on the rejected row, or failure outside case 4 is invalid RED and must be corrected before production work.

2. **Correct only the private comparison helper's cent-scalable gate.** In `amountsDifferByMoreThanOneCent(leftRounded, rightRounded)`, determine cent scalability from magnitude, not from the binary product's integer shape: both validated finite rounded values use the ordinary cent path when `Math.abs(value) <= Number.MAX_SAFE_INTEGER / 100`. In that path compute `leftCents = Math.round(leftRounded * 100)` and `rightCents = Math.round(rightRounded * 100)`, then return `Math.abs(leftCents - rightCents) > 1`. When either value is outside that magnitude gate, do not multiply by 100 or subtract against `0.01`; retain the first correction's equality-only rule, because distinct representable values at those magnitudes exceed cent resolution. Keep one private helper shared by EUR nominal/base and foreign base/rate comparisons. Do not change validation precedence, authority rules, result/audit naming, guard sequencing, merged parity, direction mapping, fixed errors, paired transfer behavior, or any M03/M04/M05 code.

3. **Rerun the complete gates and restart artifact review from zero.** Require H10 **12/12**, bank reconciliation **83/83**, full unit **2,509/2,509 across 76 files**, build/release/diff checks PASS, and integration **20 PASS/3 documented skips**. The final evidence must include all four ordinary boundaries—`0.07`/`0.06` accepted, `0.07`/`0.05` rejected, 90/89.99 accepted, 90/89.98 rejected—plus both huge foreign/EUR conflicts. Regenerate `.omc/reviews/H10.diff` from `H10_IMPLEMENTATION_BASE`, record replacement bytes/SHA-256, prove frozen/live identity, and restart fresh specification, code-quality, and verifier reviews in that order. Update the ledger with this second RED checkpoint, the magnitude-gated cent comparison evidence, final GREEN/gates, and the new immutable artifact identity. Any edit or finding invalidates the artifact and all verdicts again; do not commit H10 or begin M03/M04/M05 until the unchanged replacement artifact is approved and verified.

#### H10 third review-correction plan

The 56,203-byte H10 artifact with SHA-256 `085e11c9bcacb01a74b5c71463587149438f05b152cbd30eca3ad02f8d409d4e` failed specification review because the small-money acceptance fixture used dry-run instead of the required execute path, so it did not prove EUR `0.07` confirmation/distribution, in-run guard recording, or audit output. The paired two-cent rejection also omitted explicit empty `already_handled` and `ambiguous_refless` assertions. Invalidate that artifact and its verdict. Preserve the correct private comparison helper, exactly twelve H10 tests, and the exact two-file implementation scope.

1. **Strengthen existing H10 case 4 only, without changing production first.** Run the `amount: 1`, `base_amount: 0.07`, `currency_rate: 0.06` setup with `{ execute: true }`. Add a second independent-looking but same-key ref-less C row on the same date and dimensions with `amount: 1`, no base amount, and `currency_rate: 0.07`. Assert the first row is the only confirmed `one_sided`/`execution.results` item with normalized `currency: "USD"`, nominal `amount: 1`, `amount_eur: 0.07`, exact EUR `0.07` target distribution, and exactly one audit whose summary/details carry EUR `0.07`. Assert the second row is not confirmed and appears as the only `ambiguous_refless`/`execution.skipped` item with `amount_eur: 0.07`; this public result proves the first confirmation recorded the guard at the authoritative EUR amount without exposing or mocking private guard internals. Snapshot the acceptance audit calls and clear the spy before invoking the rejection setup.

2. **Complete the paired rejection evidence in the same test.** For `amount: 1`, `base_amount: 0.07`, `currency_rate: 0.05`, retain the exact `one_sided_eur_amount_conflict` code/reason and zero get/update/confirm/audit checks, and explicitly require `one_sided`, `already_handled`, `ambiguous_refless`, `execution.results`, and `execution.skipped` all to be empty. Do not add a thirteenth H10 test, alter the helper, change Phase 1/1b, or touch M03/M04/M05.

3. **Treat specification review as the correction RED and rerun all executable GREEN gates.** This is a test-evidence gap while production behavior is already believed correct, so do not manufacture a behavioral failure: the pre-edit executable baseline remains H10 **12/12** and bank reconciliation **83/83**. After strengthening the test, require H10 **12/12**, bank reconciliation **83/83**, full unit **2,509/2,509 across 76 files**, build/release/diff checks PASS, and integration **20 PASS/3 documented skips**, with `src/tools/bank-reconciliation.ts` byte-identical across this correction. If the strengthened case genuinely fails, the only valid behavioral RED is case 4 (**1 FAIL/11 PASS**, file **1 FAIL/82 PASS**); diagnose it before any minimal Phase-2 correction.

4. **Freeze and review a new artifact from zero.** Regenerate `.omc/reviews/H10.diff`, record its byte count/SHA-256, prove live/frozen identity, and restart fresh specification review, code-quality review, and independent verification sequentially. Any edit invalidates the replacement artifact and all verdicts again. Update the H10 ledger with this specification-review RED, the execute/distribution/guard/audit and empty-bucket evidence, final gates, new artifact identity, and fresh ordered verdicts before the original H10 implementation commit. Do not begin M03, M04, or M05.

#### H10 fourth review-correction plan

The 58,329-byte H10 artifact with SHA-256 `7a34b9e4734e7487733f344c36b37b63eba39f51732f062d2fd62c78659db8c9` passed the third correction's execute/guard/audit evidence but failed specification review because the original cases 1-3 require the exact C/D source-to-target mapping in existing-journal `already_handled` and mirrored `execution.skipped` rows. The Phase-2 matched branch currently exposes only `source_account`, omitting source dimension, target account, and target dimension. Invalidate that artifact and its verdict; preserve the twelve-test count and two-file implementation scope.

1. **Strengthen existing H10 cases 1-3 before production edits.** In case 1 require both `already_handled[0]` and `execution.skipped[0]` to independently contain source Wise/300 and target LHV/100. In case 2 require both rows to independently contain source LHV/100 and target Wise/300. In case 3 create and invoke every C/D dry-run, execute, and existing-journal setup, collect both directions' handled payloads before asserting either direction, then use aggregate or soft assertions so a missing C mapping cannot mask D. For both rate-only directions require the exact four endpoint fields independently on `already_handled` and `execution.skipped`; do not merely compare one incomplete runtime row with the other. Do not add a thirteenth H10 test.

   Expected correction RED is exactly H10 **3 FAIL/9 PASS** and bank reconciliation **3 FAIL/80 PASS**, with only the foreign-base C, foreign-base D, and rate-only both-direction tests failing solely on the four absent endpoint fields. If the full unit suite is run at RED, expect **3 FAIL/2,506 PASS**. Any resolver, mutation, audit, paired-flow, fixture, or unrelated failure is invalid RED.

2. **Add the mapping once in the Phase-2 matched-journal object.** Extend the shared private `skippedAlreadyHandled` item type with additive optional `source_dimension_id?: number`, `target_account?: string`, and `target_dimension_id?: number` fields so unchanged Phase 1/1b pushes remain compatible. In only the Phase-2 `resolution.status === "matched"` push, populate those three fields unconditionally from `tx.accounts_dimensions_id`, `targetTitle`, and `targetDimension`; retain the existing `source_account`. Because `execution.skipped` consumes the same object, do not add a second transformation. Do not change amount resolution, BookingGuard lookup/consumption, client lookup, confirmation, mutation, audit, dry-run results, `ambiguous_refless`, Phase 1/1b, or any M03+ code.

3. **Rerun all gates and restart review from zero.** Require H10 **12/12**, bank reconciliation **83/83**, full unit **2,509/2,509**, build/release/diff checks PASS, and integration **20 PASS/3 documented skips**. Regenerate `.omc/reviews/H10.diff`, record bytes/SHA-256, prove frozen/live identity, and run fresh specification review, code-quality review, and independent verification sequentially. Any edit invalidates the replacement artifact and verdicts. Update the ledger only after approval, then use the original H10 implementation commit; do not begin M03/M04/M05.
...
#### M03 execution-plan hardening

The original sketch correctly rejects identifier-only trust but leaves the ownership evidence, dimension binding, explicit approval, dry-run review output, read/mutation ordering, and interaction with the existing transfer tests underspecified. Execute this hardening plan instead of the original Steps 1-4; retain the original M03 implementation commit message. A `TRANSFER-*` or `BANK_DETAILS_PAYMENT_RETURN-*` identifier is only a candidate hint. The ordinary Wise bank transaction may still be created as a PROJECT row, but inter-account journal lookup, client attachment, confirmation, and audit require either verified endpoints or an exact explicit approval after the source and target dimensions themselves pass structural verification.

1. **Commit this approved plan as the clean M03 implementation base.** The H10 code baseline is `af62766`. Commit only this plan document with `git commit -m "docs(M03): harden Wise transfer ownership plan"`, record that commit as `M03_IMPLEMENTATION_BASE`, and require a clean worktree before any source/test edit. Work only in `src/tools/wise-import.ts` and `src/tools/wise-import.test.ts`; do not change shared API types, `BookingGuard`, CAMT/bank reconciliation, or begin M04/M05. Measured baseline at `af62766`: Wise import **37/37**, full unit **2,509/2,509 across 76 files**, build/release/diff checks PASS, integration **20 PASS/3 documented skips**.

2. **Write exactly eight handler-level `M03` cases before production edits, while preserving existing transfer tests with truthful fixtures.** Add a test helper that supplies configured Wise/other-bank account records with `account_name_est`/`account_name_eng`/`beneficiary_name` identities rather than relying on bare dimension IDs. Update existing transfer fixtures that intentionally confirm or find an existing journal so both endpoints remain verified under the new contract; the existing no-invoice-company control must prove verification from configured account identities, not retain prefix-only trust. Add exactly these selected cases:

   1. `M03 leaves prefix-only transfers in ownership review`: table over IN/OUT and both identifier families with zero-fee independent setups. Execute may create the ordinary PROJECT row but must return one fixed ownership-review item and make zero journal/guard/client/update/confirm calls.
   2. `M03 binds endpoint identities to two distinct configured bank dimensions`: table over an identity belonging only to an unrelated third dimension, an identity reused across dimensions, a missing/unconfigured Wise dimension, and an explicit missing/same/non-bank target. Structural failures are non-overridable. Include an identity-read rejection and prove the hinted batch aborts before every transaction create.
   3. `M03 validates exact explicit transfer approvals before mutation`: prove an exact approval confirms an otherwise ownership-only-unverified row and reports `operator_approved`; redundant approval of already verified endpoints reports `verified_endpoints`. Duplicate approval IDs reject with `confirm_own_transfer_ids must contain unique exact Wise transfer IDs.`; unknown, non-transfer, and case-mismatched IDs reject with `confirm_own_transfer_ids must reference eligible TRANSFER-* or BANK_DETAILS_PAYMENT_RETURN-* rows in this CSV exactly.`. Collect independent outcomes so one failure cannot mask another, and require zero create/update/confirm/audit for rejected approval input.
   4. `M03 dry-run exposes ownership review without synthesizing approval`: an unverified hinted row remains `would_create`, appears in `execution.needs_review`, performs no mutation/guard read, and the workflow suggestion never invents `confirm_own_transfer_ids`; caller-supplied exact approvals may be preserved but never manufactured.
   5. `M03 exposes a compact approval contract and fixed non-echo review text`: metadata names `confirm_own_transfer_ids` and states that prefixes are hints. Instruction-like endpoint names never occur in a review reason; reason/code/booleans are fixed, while raw endpoint names are omitted from review output.
   6. `M03 does not let approval override invalid bank dimensions`: even an exact approved ID remains in structural review when the Wise source dimension or other-bank target is absent, non-bank, identical, or ambiguous; no journal/client/update/confirm work occurs.
   7. `M03 control verifies both endpoint directions and transfer prefixes`: table over IN/OUT and both identifier families with two distinct configured bank dimensions and exact endpoint evidence; current intended confirmation/already-journalized behavior remains a pre-change PASS control.
   8. `M03 control leaves non-transfer import behavior unchanged`: an ordinary payment remains created/previewed without ownership reads or review; use existing zero-fee/Jar/duplicate/fee tests as the broader unchanged regression suite rather than mixing fee confirmations into ownership call-count assertions. This is the second pre-change PASS control.

   Require selected correction RED exactly **6 FAIL/2 PASS out of 8** and whole Wise file **6 FAIL/39 PASS out of 45**. The only passing selected controls are cases 7-8. Any failure caused by fixture shape, existing transfer behavior that should be verified, fee side effects, import/CSV parsing, or a non-M03 test is invalid RED and must be fixed before production work.

3. **Add one exact approval input and validate it against eligible row identities before any API mutation.** Add `confirm_own_transfer_ids: z.array(z.string().min(1)).optional()` with a compact description. Treat approval IDs as case-sensitive opaque strings; reject duplicates with the first fixed message above and reject every ID not exactly equal to an eligible candidate row whose ID starts with `TRANSFER-` or `BANK_DETAILS_PAYMENT_RETURN-` with the second fixed message. Do not echo an offending ID. Validation happens after CSV/date/status filtering but before transaction list/create/update/confirm, client, journal, or audit work. Keep ownership decisions keyed by the `WiseRow` object or row index, never only by `wise_id`, so duplicate CSV IDs cannot merge row decisions or alter existing in-run duplicate behavior.

4. **Preflight authoritative identities and dimension structure once before creation for every hinted batch.** When no eligible hinted row exists and no approval is supplied, do not add bank-account or invoice-info reads. Otherwise call `api.readonly.getBankAccounts()` and `api.readonly.getInvoiceInfo()` exactly once each before any transaction creation; reuse both snapshots for target auto-detection, ownership classification, and later client work. If either read fails, abort the entire hinted batch before all creation—this is the deliberate exception to ordinary PROJECT creation. Require `accounts_dimensions_id` to resolve to a configured bank-account dimension and the target to resolve to one distinct configured bank-account dimension: use a valid explicit `inter_account_dimension_id`, otherwise the unique distinct other bank dimension. Missing, same, non-bank, or ambiguous dimensions produce code `wise_transfer_dimensions_unverified`, reason `Wise and target dimensions must resolve to two distinct configured bank accounts before reconciliation.`, and `approval_required: false`; approval cannot override this structural failure.

   Build normalized endpoint identities only from the exact invoice-company name and name-bearing fields on configured bank accounts: `beneficiary_name`, `account_name_est`, and `account_name_eng`. Do not treat `bank_name`, account number, or identifier-prefix shape as ownership evidence. Collapse duplicate records by dimension and fail closed when a non-company identity is reused across dimensions. For IN, verify CSV source against the other-bank dimension and target against the Wise dimension; for OUT, verify source against Wise and target against other-bank. Exact normalized invoice-company identity may prove either endpoint. Preserve truthful direction-aware `source_verified`/`target_verified` booleans even when structural verification fails.

5. **Gate only inter-account actions on one precomputed decision and expose a safe review contract.** For structurally valid rows, both endpoints verified yields `ownership_basis: "verified_endpoints"`; this takes precedence even if the ID was redundantly approved. An exact approval may override only an ownership failure and yields `ownership_basis: "operator_approved"`. Otherwise produce code `wise_transfer_ownership_unverified`, reason `Wise transfer ownership is unverified; both endpoints must match configured own-account identities or this exact Wise ID must be explicitly approved.`, `approval_required: true`, and the truthful endpoint booleans. The review object contains `wise_id`, fixed code/reason, booleans, and approval flag, but no raw endpoint names. The ordinary created/would-create bank row stays in `results`; the review object is additionally passed once to `buildBatchExecutionContract({ needs_review })`, counted in `summary.needs_review`, and may be exposed in a matching top-level ownership-review array. Fixed reasons are never constructed from CSV text.

   Only decisions with a structurally valid target and `ownership_basis` enter `transferEntries`. Unverified zero-fee entries perform no account-dimension, journal/guard, client, update, or confirm work. Add required `ownership_basis: "verified_endpoints" | "operator_approved"` to every inter-account result status, including `already_journalized`, `confirmed_inter_account`, and `confirm_failed`; include it in the confirmation audit details where an audit event exists. Reuse the cached bank/account/company snapshots and preserve all existing amount, reference, guard, orphan warning, create, fee, Jar, duplicate, invoice-fix, progress, wrapper, and output behavior otherwise.

6. **Keep M03 separate from M04.** Dry-run must classify and surface unverified candidates, but do not add command generation, EUR provenance redesign, approval digests, or execute-time command replay. Workflow arguments must never synthesize `confirm_own_transfer_ids`; include caller-supplied IDs only when they were explicitly present. Preserve existing dry-run results and mutation-free behavior apart from the new ownership review/read-only classification.

7. **Prove GREEN, freeze one artifact, and review sequentially.** Require M03 **8/8**, Wise import **45/45**, full unit **2,517/2,517 across 76 files**, `npm run build`, `npm run validate:release`, `git diff --check`, and integration **20 PASS/3 documented skips**. Require the implementation diff to contain exactly `src/tools/wise-import.ts` and `src/tools/wise-import.test.ts`. Freeze `.omc/reviews/M03.diff`, record byte count/SHA-256, prove frozen/live and later staged identity, then obtain fresh ordered specification, code-quality/security, and independent-verifier approvals. Any edit invalidates the artifact and verdicts. Append the ignored ledger row with baseline/RED/GREEN, identity/dimension/approval/review/mutation evidence, artifact identity, verdicts, and final commit; then use `fix(M03): verify Wise transfer ownership`. Do not begin M04 until M03 is committed and clean.

#### M03 pre-artifact review-correction plan

The first GREEN implementation passed all 45 Wise tests but adversarial pre-review found that BankAccount dimension membership alone was treated as posting structure. A configured bank target whose `AccountDimension` is missing/deleted silently leaves an imported PROJECT transaction with no review or recovery result, and malformed runtime dimension/identity values can enter ownership matching. No M03 artifact has been frozen. Preserve the eight-test count and two-file scope; commit this correction plan before changing tests.

1. **Strengthen existing M03 case 2 only before production edits.** Add independent rows for a configured bank target whose posting dimension is missing or deleted, a Wise source posting dimension that is missing or deleted, explicit target dimension values that are zero, negative, fractional, or runtime non-number, an auto-detected malformed bank dimension, and malformed numeric/object `beneficiary_name`/account-label values. Extend the existing identity-read failure table with `getAccountDimensions()` rejection. Collect every result/error before assertions. Every structural row must return the fixed `wise_transfer_dimensions_unverified` review with `approval_required: false`, truthful direction-aware endpoint booleans, and zero journal/guard/client/update/confirm work; malformed identity text must fail closed without throwing or becoming evidence. Every preflight read failure must abort before transaction list/create/audit. Do not add a ninth selected test. Expected correction RED is only case 2: M03 **1 FAIL/7 PASS**, Wise **1 FAIL/44 PASS**.

2. **Validate both configured-bank identity and active posting structure before creation.** Add one private positive-safe-integer runtime guard. Accept a BankAccount identity only when `accounts_dimensions_id` is a finite positive safe integer; normalize identity inputs as `unknown` and return empty for every non-string rather than calling string methods. Build endpoint ownership evidence from those safe bank records alone, preserving truthful booleans even when later posting structure is invalid. Build structural dimensions from the intersection of configured bank dimensions with exactly one active `AccountDimension` row whose `id` and `accounts_id` are finite positive safe integers. Missing, deleted, malformed, duplicate, or conflicting active posting rows for an ID fail closed; never select the first ambiguous row.

3. **Preflight and cache all three authoritative snapshots for hinted batches.** Every eligible hinted batch calls `getBankAccounts()`, `getInvoiceInfo()`, and `getAccountDimensions()` exactly once before transaction list/create/audit, aborting the entire hinted batch if any read rejects. Reuse the same account-dimension snapshot for fee resolution and later reconciliation, so hinted fee rows do not perform a second read. This explicitly supersedes the earlier statement that an unverified zero-fee row performs no account-dimension read; it still performs no journal/guard/client/update/confirm work. Non-hinted zero-fee imports add no read, and non-hinted fee imports retain their existing single lazy account-dimension read.

4. **Harden schema and runtime paths together.** Change `inter_account_dimension_id` to the repository's positive-integer `coerceId.optional()` schema. Registered calls reject malformed values at the boundary, while direct-handler/runtime malformed values must still be classified into the same fixed non-overridable structural review without mutation. Require both the Wise source and one distinct target to survive the safe configured-bank/active-posting intersection before either verified endpoints or operator approval may reconcile. Reuse the validated posting-dimension map for distribution; never perform a later first-match lookup that can silently fail.

5. **Rerun all gates and restart pre-artifact review.** Require M03 **8/8**, Wise **45/45**, unit **2,517/2,517**, build/release/diff PASS, and integration **20 PASS/3 skips**. Re-run adversarial review before freezing `.omc/reviews/M03.diff`; any finding/edit restarts the artifact sequence. Do not implement M04/M05.
...
#### M04 execution-plan hardening

The original sketch identifies the missing inter-account preview, but its generic main-row digest omits other mutations performed by the same tool, material create/confirm fields, connection and file identity, fresh-state requirements, existing-journal outcomes, output trust boundaries, and the shipped workflow contract. Execute this hardening plan instead of the original Steps 1-4; retain the original M04 implementation commit message. `approved_command_digest` approves the complete mutation plan for one `import_wise_transactions` call. It must never imply exact approval while fee confirmation or invoice updates remain outside the digest.

1. **Commit this approved plan as the clean M04 implementation base.** The M03 code baseline is `f657cd3`. After plan review, commit only this plan document with `git commit -m "docs(M04): harden Wise command parity plan"`, record the resulting hash as `M04_IMPLEMENTATION_BASE`, and require a clean worktree before tests or production edits. Implementation scope is `src/tools/wise-import.ts`, `src/tools/wise-import.test.ts`, `src/workflow-response.ts`, `src/workflow-response.test.ts`, `workflows/import-wise.md`, its generated `.claude/commands/import-wise.md` mirror, and the existing Wise prompt assertions in `src/prompts.test.ts`. Do not change shared API types, `BookingGuard`, CAMT/bank reconciliation, or begin M05. Baseline at `f657cd3`: Wise import **45/45**, full unit **2,517/2,517 across 76 files**, build/release/diff checks PASS, integration **20 PASS/3 documented skips**.

2. **Write exactly ten handler-level `M04` cases before production edits and update conflicting legacy execute fixtures only after the focused RED is captured.** Add a test helper that performs the real dry-run, extracts its digest, clears read/mutation/audit spies without changing mocked upstream state, and invokes execution with that exact digest. Use it only to migrate existing successful `execute: true` expectations after RED; missing/malformed/stale-digest cases must call the raw handler. Expected focused RED is exactly **8 FAIL/2 PASS out of 10**, and the whole Wise file is **8 FAIL/47 PASS out of 55**. The two passing controls are cases 9-10. Any import/transform failure, fixture-only failure, or unrelated regression is invalid RED.

   1. `M04 previews exact IN and OUT inter-account actions`: table both identifier families and directions. Require stable source-row identity, `C`/`D`, flow source/target bank dimensions, target posting account/dimension, booked and Wise source/target amounts and currencies, exchange-rate orientation, M03 ownership basis, dependency, and exact mutation mode. In the same failing case, require an ownership-review and a structurally invalid transfer to retain their digest-bound ordinary main-create command while adding no inter-account dependent command. Dry-run makes zero create/update/confirm/invoice-update/audit calls.
   2. `M04 distinguishes create-confirm from an existing journal`: a fresh journal snapshot produces either a planned create plus optional client update plus confirmation distribution, or a create-only/already-journalized action with the exact existing journal ID and no client/confirm action. Include repeated same-key rows so the preview simulates the in-run guard records execution would create.
   3. `M04 covers every Wise mutation category in execution order`: one batch exercises ordinary main creation, a transfer main creation and confirmation, fee creation and confirmation, and a purchase-invoice FX update. The immutable plan is ordered by the current phases and binds dependencies; a missing Wise fee client is detected during preview and blocks approval rather than producing a plan execution cannot complete.
   4. `M04 binds complete input, monetary, target, and live-state provenance`: independently vary exact CSV bytes, source row index/duplicate ID placement, filters, skip-Jar option, source and target dimensions, account/posting IDs, direction, dates, nominal/base amounts and currencies, rate orientation, client resolution, fee distribution, invoice current state/patch, duplicate transaction state, and journal state. Each material change changes the digest.
   5. `M04 canonical digest is deterministic and connection scoped`: recursively sorted keys, explicit `undefined` to `null`, preserved command order, a fixed version/domain separator, exact CSV SHA-256, and `api.transactions.connectionFingerprint` produce the digest. Equal fresh plans match; command order, any command field, CSV bytes, or fingerprint changes it. Random sandbox wrapper nonces and public display projection are excluded.
   6. `M04 rejects missing malformed stale and misapplied digests before mutation`: missing, uppercase, wrong-length, whitespace, non-string, another connection/file/argument/state plan, a digest supplied for an empty plan, and a changed plan all return the fixed structured error below before progress, create, update, confirm, purchase-invoice update, or audit. Collect independent outcomes before assertions.
   7. `M04 executes the approved immutable plan exactly once`: exact digest execution interprets the same internal command array compiled before the branch, calls the complete stored payloads in order, never re-derives amount/dimension/ownership/mode in the executor, and returns the same stable public `execution.commands` projection and digest as dry-run. Runtime-created IDs may appear only in the existing result/reconciliation fields.
   8. `M04 preserves partial-failure and audit truth`: main-create failure suppresses dependents; create success followed by confirm failure retains the existing orphan PROJECT ID/recovery hint; audits occur only after the corresponding successful mutation and include command version/digest without recording future actions.
   9. `M04 control preserves M03 review and skip behavior`: assert only the existing pre-M04 contract so this remains a genuine pre-change PASS—ownership/structural review text and booleans remain fixed, already-imported rows remain skipped without review leakage, ambiguous invoice candidates remain `ambiguous_skipped`, filtered input stays empty, and dry-run makes no remote mutation/audit. Do not assert new command/digest fields in this control; case 1 and case 3 carry the load-bearing category-specific M04 command assertions.
   10. `M04 control preserves ordinary dry-run presentation`: current summary/results/invoice-fix preview and safe output remain unchanged apart from additive command/digest fields; no M05 row-validation behavior is introduced.

3. **Compile one complete internal command plan before the dry-run/execute branch.** Use a versioned discriminated `WiseImportCommand` union for main transaction creation, fee create-and-confirm, inter-account client/confirmation or known-existing-journal handling, and purchase-invoice FX updates. Preserve the current runtime phase order and model dependencies by stable command IDs derived from CSV row index plus action kind, never by Wise ID alone. Each internal command contains the complete mutation payload it will pass to the API, its dependency, source-row index, raw Wise ID, authoritative amounts/currencies and rate orientation, source/target bank and posting dimensions, ownership basis, current-state expectation, and relevant existing object IDs. Read the resolved file as a `Buffer`, hash those exact original bytes before UTF-8 decoding, and include that SHA-256, the connection fingerprint, and canonical non-execution planning arguments in the approval snapshot. M03 ownership-review and structurally invalid transfer rows retain their ordinary main-create command but add no inter-account dependent command; an already-imported main or fee omits only that existing mutation, an ambiguous invoice candidate omits only its invoice-update command, and fully filtered input has no commands.

   Compile from fresh read-only snapshots before any mutation: deliberately call the existing `clearRuntimeCaches()` once before planning, then use uncached `listAll()` for transactions, clients, and relevant purchase invoices and load journals/reference data after that flush. This process-wide flush clears every connection rather than only the active one; accept and document the bounded performance/concurrency tradeoff because generation checks discard in-flight stale cache writes and correctness is more important at an approval boundary. Test that the flush precedes every planning read, that same-call snapshots are reused, and that changed upstream mocks are observed on execute. Both dry-run and execute use this same compiler. Simulate `BookingGuard.recordInterAccount` for planned confirmations so later same-batch commands match execution. A read or precondition failure aborts the batch before any mutation/audit. Do not add or change shared cache/API interfaces merely for M04.

4. **Canonicalize and gate the whole plan.** Add `approved_command_digest: z.string().regex(/^[0-9a-f]{64}$/).optional()` and defensively reject invalid runtime values before API work. `digestWiseCommands` uses SHA-256 over a domain-separated v1 envelope containing connection fingerprint, exact raw-file SHA-256, canonical planning arguments, and the recursively key-sorted command array; normalization maps `undefined` to `null` and preserves command order. Planning arguments include exact `file_path`, selected/resolved bank and fee dimensions, exact explicit transfer approvals in canonical order, date filters, and Jar policy, while excluding only `execute` and `approved_command_digest`. When at least one mutation command exists, execution requires an exact digest. A supplied digest with no executable command is stale/misapplied and also rejects. There is no legacy execute bypass; update all current successful execute tests through the two-pass helper and update the shipped workflow to instruct callers to use the digest returned by dry-run.

   Missing, malformed, stale, changed, or misapplied approval returns `toolError` with exactly one fixed non-echo contract: `category: "digest_mismatch"`, `code: "approval_digest_mismatch"`, `mutation_occurred: false`, `known_object_ids: []`, `affected_cache_names: []`, and `next_action: "Run a new Wise dry run, review its complete command plan, and approve that exact digest."`. Do not return the expected digest, the supplied digest, raw command differences, connection identity, or untrusted CSV text; an execute error must never provide a replacement approval token.

5. **Separate the raw approval snapshot from the public command projection.** Digest and execute the complete raw internal commands, but expose only a deterministic, additive `execution.commands` projection containing action/mutation mode, stable row/action identity, type, amounts, currencies, rate orientation, dimensions/accounts, ownership basis, dependency, and expected existing-object IDs. Do not add raw CSV names, references, descriptions, notes, or IDs as trusted prose; use stable hashes where row identity is necessary. Wrapper nonces and display-only fields never participate in the digest. Dry-run and unchanged-state execution must return identical public commands and `approved_command_digest`; existing results may additionally carry runtime API IDs and statuses.

6. **Execute only the approved commands and retain current failure semantics.** After exact digest validation, one executor consumes the compiled array in order. It stores runtime IDs under stable dependency keys; failed prerequisites suppress dependents. It uses stored create/update/confirm/invoice payloads verbatim and does not rerun ownership, amount, dimension, client, fee, journal, or invoice decisions. Record each existing audit only after the corresponding mutation succeeds, optionally adding command version/digest. Preserve duplicate handling, fee/main dependency, already-journalized behavior, M03 reviews, confirm-failure orphan recovery, invoice ambiguity, summary/result fields, and current non-atomic partial-failure reporting.

7. **Carry the approval through every workflow surface, including invoice-only plans.** Add `command_count` only to `dry_run_steps[].preview` (and optionally the top-level display summary); add only `approved_command_digest` to `suggested_args` so the existing generic `withExecuteTrue` transformation forwards a schema-valid execution call. In `src/workflow-response.ts`, retain the existing created-count fallback for older Wise previews but let a positive preview `command_count` make a zero-bank-create invoice-update-only plan approvable when there are no errors. Inside the existing materializing-dry-run test—do not add another unit-test count—build a second independent envelope containing only one Wise step whose `created` is zero, preview `command_count` is positive, and invoice FX update is present; require its `approval_previews[].execute_args`, available action, and recommended action all to carry the exact digest and explicitly omit `command_count`. Update `workflows/import-wise.md`, regenerate `.claude/commands/import-wise.md` with `npm run sync:workflow-prompts`, and strengthen the existing prompt test so “same arguments” explicitly includes `approved_command_digest` from the reviewed dry run. Require the generated mirror to equal the source after removing its standard two-line generated-file header; do not hand-edit or alter unrelated prompts.

8. **Prove GREEN, freeze one artifact, and review sequentially.** Require M04 **10/10**, Wise import **55/55**, full unit **2,527/2,527 across 76 files**, workflow/prompt sync, `npm run build`, `npm run validate:release`, `git diff --check`, and integration **20 PASS/3 documented skips**. Require exactly the seven scoped files in the implementation diff. Recheck full-plan mutation coverage, connection/file/fresh-state sensitivity, no replacement digest disclosure, untrusted-output projection, invoice-only workflow propagation, exact plan execution, partial failures, M03/H10 compatibility, and absence of M05 validation. Freeze `.omc/reviews/M04.diff`, record byte count/SHA-256, prove frozen/live and staged identity, then obtain fresh ordered specification, code-quality/security, and independent-verifier approvals. Any edit invalidates the artifact and all verdicts. Append the ignored ledger row and commit with `fix(M04): align Wise preview and execution`; do not begin M05 before M04 is committed and clean.
