...
function risk(name: string): ToolRisk {
  ...
  if (new Set("preview_purchase_invoice_totals_correction reconcile_transactions prepare_year_end_close scan_receipt_folder classify_unmatched_transactions accounting_inbox prepare_accounting_review_action".split(" ")).has(name)) return "preview";
