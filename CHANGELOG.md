...
- **CAMT import keeps a second identical entry without a bank reference.** Two genuine identical same-day entries without `AcctSvcrRef` were collapsed into one; the ledger count now decides how many copies are flagged as possible duplicates. The closing-balance tripwire now also works on the dry run (`pending_import_amount`), and execute reads the ledger once instead of once per row.
