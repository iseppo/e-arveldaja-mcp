...
- **Supplier resolution misses corporate-form variants** — `LEGAL_SUFFIXES` in `company-name.ts` now strips `Corp`, `Corporation`, `Co`, `LP`, `LLP`, `PLC`, `PBC`, `PLLC`, `AG`, `SAS`, `SARL`, `SRL`, `SPA`, `NV`, and `BV` in addition to the existing Estonian / Baltic / German set. `resolveSupplierInternal` gains a `name_normalized` match tier that runs before the fuzzy fallback: an invoice supplier "Anthropic, PBC" now resolves to an existing "Anthropic" client (the fuzzy 0.7 threshold rejected the pair at ≈0.62), so `suggestBookingInternal` reuses prior bookings instead of falling through to the keyword fallback.
