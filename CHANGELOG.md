...
- **Action endpoints get the same protection.** Sale-invoice e-invoice delivery, confirm/invalidate of sale invoices, purchase invoices, journals and transactions, and client/product deactivate/reactivate bypassed the shared mutation wrapper, so a dropped connection was not flagged as possibly applied (a retried e-invoice could be sent twice). All now go through it.
- **Confirming a payment refreshes invoice caches.** Confirming or invalidating a bank transaction changes invoice payment status but left `/sale_invoices` and `/purchase_invoices` cached for up to 2 minutes, so a following auto-confirm or import could pay the same invoice again. Those caches (and journals) are now cleared, also when the outcome is unknown. `listAll` drops rows repeated across pages.
- **Checks that gate a write now read live data.** Supplier/customer auto-create (`resolve_supplier`, sale-invoice inline customer), exact-match auto-confirm, inter-account execute, receipt batch, transaction classifications apply, `execute_year_end_close`, currency rounding, `cleanup_camt_possible_duplicate`, the intake cash-duplicate check, CRUD `block_on_duplicate` scans and sale-invoice update status checks read from the cache and could act on data changed in the UI or by another flow. Previews still use the cache. The receipt inbox now also runs the intake cash-duplicate check (advisory) at execute, not only on dry run.
- **`update_transaction` keeps import identity markers.** Changing a description dropped the `WISE:` prefix, the `[source_direction=…]` marker and the CAMT identity line, so the row could be imported again and the confirm-time direction guard was lost. They are now preserved; `ref_number` is canonicalized like on create; a description that would exceed 150 characters is refused (`description_too_long`); an edit that would break a hash-only CAMT identity is refused (`camt_identity_would_break`).
- **`update_purchase_invoice` validates items and reads live.** New items get the same non-VAT, VAT-default and dimension checks as on create, and the invoice is re-read before merging so UI edits are not reverted. VOID invoices can no longer be edited (`void_record_immutable`); draft `update_journal` / `update_sale_invoice` are validated like create, and journals must balance on create and update.
- **`create_client` refuses duplicates.** A client with an existing registry code, or the company's own code/VAT number, is refused (`duplicate_client` / `client_self_match`, with the existing ids) unless `allow_duplicate: true`. `update_client` identity changes now go through the same legal-entity check as create.
- **CRUD details.** Tool-only flags (`block_on_duplicate`, `allow_duplicate`, …) are no longer sent to the API; batch tools report unknown outcomes as `indeterminate` (audited) instead of `failed`; the `create_journal` duplicate scan compares EUR amounts; the server-managed field denylist covers the full field list.
- **Owner-expense receipts can be completed from the inbox.** An owner-paid receipt in `needs_answers` had no way to receive the answers. `continue_accounting_workflow` `prepare_action` now takes them in `review_item_json.item.owner_expense`, lists what is still missing (each review question is answered separately in `question_answers` with `true`/`false`, never free text; per question the answer proceeds, limits the VAT deduction — e.g. an M1 car not excluded from private use cannot take a full deduction — requires no deduction (guest reception, staff personal consumption, KMS § 30 lg 1), or blocks booking (e.g. the document does not identify the seller)), and on completion goes through the normal approval and `plan_handle` execute flow. The payable account is always asked explicitly (default 2110; a per-person-dimension account such as 1360 cannot be used here).
- **Workflow prompts and server instructions match the active profile.** They named tools that do not exist on the default `standard` profile (`process_bank_input`, `run_accounting_report`, …) or on guided (`confirm_*`, Lightyear, new-supplier). Prompts now render only the sections whose tools are registered and are not registered at all when none of their tools are; a test pins this for every profile. Also fixed: `receipt-batch` and `classify-unmatched` now pass `plan_handle` (and choose groups before the dry run), the guided `book-invoice` flow, the inter-account execute mode in `reconcile-bank`, the Wise approval-digest location, and the mixed-client / third-party payer note.
...
- **Deactivate/reactivate are audited as such.** `deactivate_client` / `deactivate_product` log `DEACTIVATED` (was `DELETED`) and the reactivate tools `REACTIVATED` (was `UPDATED`); `get_session_log` accepts both as filters.
- **`prepare_dividend_package` defaults to `dry_run: true`.** Drafts are created only with an explicit `dry_run: false` after approval.
- The receipt batch dry run's `next_action.args` no longer carries the non-input `approved_manifest_required` flag, so the args can be resent verbatim.
...
- **"Today" is now the Estonian calendar date, not the UTC date.** Between local midnight and 02:00/03:00 the UTC date was still yesterday, which shifted the month-end open-month cut-off, the default `as_of_date` of `compute_receivables_aging` / `compute_payables_aging` and the `run_accounting_report` aging op, the default end of the `receipt_client_alignment` window, and the fallback journal date in currency rounding. All now use a shared `todayInTallinn()` (Europe/Tallinn).
- **Profit and loss no longer shows zeros for a closed year.** `compute_profit_and_loss` and `run_accounting_report` (`profit_and_loss`) summed the `YECL-YYYY` closing journal that older versions of `execute_year_end_close` booked on 12-31, which nets every revenue and expense account to 0 — so after the close the year's P&L read 0 / 0 / 0. Year-end closing journals are now excluded from P&L (with a warning naming how many); the balance sheet and trial balance still include them, since there they are real postings.
- **`run_accounting_report` statements now carry the opening-balance (algbilanss) warnings.** The `trial_balance`, `balance_sheet` and `profit_and_loss` ops dropped the "opening balances applied / not captured / outside this range" and unmapped-account warnings that the standalone `compute_*` tools emit. Both now share one implementation and emit identical warnings for the same dates. The unresolved-dimension-label warning, which no path ever passed through, now fires too.
- **Balance sheet without `date_to` is now as of today.** `compute_balance_sheet` and `run_accounting_report` (`balance_sheet`) documented "default: today" but applied no end date, so future-dated journals were included and the report said `date: "current"`. The default is now today's date (Europe/Tallinn), reported as a concrete `date`. A trial balance without `date_to` still counts every posting, but now says `period.to: "unbounded"` instead of the misleading `"now"`.
- **Month-end no longer counts unpaid credit invoices as receivables.** `month_end_close_checklist` and `run_accounting_report` (`month_end`) listed an unpaid `CREDIT_INVOICE` under `overdue_receivables` with a positive amount, inflating what customers owe. Credit invoices (money owed *to* the customer) are now left out of the receivables lists, with a warning giving the count.
- **Invalid report dates are rejected instead of producing wrong reports.** `compute_receivables_aging` / `compute_payables_aging` accepted any `as_of_date` string, putting every invoice in "90+" (or failing with a `RangeError`). They, and `compute_trial_balance` / `compute_balance_sheet` / `compute_profit_and_loss`, now require a real `YYYY-MM-DD` date and reject `date_from` after `date_to` (`category: "invalid_date"`), matching `run_accounting_report`.
- **`detail="full"` now returns every aging and month-end row.** The shared aging and month-end cores kept only the top 10 rows per aging bucket, party list and overdue/due-before list, so `run_accounting_report` with `detail="full"` silently dropped rows and compact mode never marked them. The cores now return full lists; the report caps compact output at 25 rows and marks every capped list with `truncated` (including the `unconfirmed_*` lists; capped aging party lists add `top_parties_truncated` and `party_count`). The standalone aging and month-end tools keep their 10-row cap but now flag it (`truncated`, `top_debtors_truncated` / `debtor_count`, `top_creditors_truncated` / `creditor_count`).
- **Foreign-currency invoices without a EUR amount are no longer added to EUR totals.** When a non-EUR invoice had no `base_gross_price`, aging and month-end summed its foreign-currency `gross_price` as if it were EUR. Such invoices now stay in the row lists, marked with `currency` and `excluded_from_eur_totals`, but are left out of every EUR total (bucket, party, unmatched, overdue/due-before), with a warning listing them.
- **Month-end and aging no longer crash on an invoice without `create_date`.** An open invoice with a missing or empty `create_date` aborted the month-end checklist (and aging, for an empty string) with a `RangeError`. The due date now falls back to the evaluation date, and month-end warns about such invoices. The month-end PARTIALLY_PAID warning now also counts invoices in the `due_before_month_end_*` lists.
- **Annual report income statement follows the real e-arveldaja chart.** `generate_annual_report_data` mapped Schema 1 lines to invented ranges (sales 3000-3099, depreciation 4800-4899, financial 7200-7699), so on the standard chart sales on 31xx-36xx fell out of net profit, 4800 goods costs were reported and added back as depreciation, and fines landed under financial items. Lines now follow the standard chart (revenue 3100-3799, other income 3800-3999, goods/services 4000-4999, operating expenses 5000-5999, staff 6000-6999, depreciation 7000-7099, other expenses 7100-7999, financial 8000-8899 net, income tax 8900-8999), checked against the account type. Any other Tulud/Kulud account (e.g. 3000 clearing) goes to a new `kaardistamata_tulud_ja_kulud` line with a warning, so net profit always equals Tulud − Kulud (excluding 9000, see below) as in `prepare_year_end_close` and `compute_profit_and_loss`.
- **First-year annual report starts from the opening balance.** With an algbilanss dated inside the report year, cash flow and ROE started from zero, giving a reconciliation difference equal to the opening position. The opening position is now the start-of-period balance, and a warning appears whenever the cash-flow statement does not reconcile.
- **Annual report classification on the real chart.** Accruals are flagged from 2600-2799 liabilities (29xx is equity); 21xx-27xx liabilities are current and 28xx non-current; 13xx are receivables and 11xx short-term investments in the cash flow; the opening-balance note says when a stored algbilanss falls after the report year; staff and related-party names, YECL document numbers and draft purchase numbers are marked as untrusted text. When the closing-journal create outcome is unknown, `execute_year_end_close` no longer says "No journals were created" and asks to check for a `YEC-RESULT-YYYY` / `YEC-RETAINED-YYYY` draft first; audit entries are logged under `execute_year_end_close`.
- **Year-end close never books twice.** Hand-booked RIK closing entries (any document number, draft or registered) and legacy `YECL-YYYY` journals that post to 2970 are detected; only the missing entry is proposed or created, and the new `close_status` reports open / partially_closed / closed. A 2970 → 2960 transfer booked by hand on any date of the next year (e.g. 1 December) counts as entry 2 when its amount equals the result; a transfer that puts the whole profit into reserve capital (D 2970 / K 2940, no 2960 line) counts too. Both entries are checked for direction and amount: a partial, reversed or stale entry is reported as `mismatch` / `partially_closed` in `blocked_entries` with booked vs expected amounts; only the remainder of a smaller same-direction 1 January transfer is proposed, and booked only with `allow_additional_transfer=true`; an off-date, ambiguous, over-, reversed or duplicate transfer (e.g. a second 2970 → 2960 transfer later in the year beside a complete 1 January one) is left for manual review and the year is not reported closed. `prepare_year_end_close` also warns when an earlier year is not closed or transferred. Re-running `execute_year_end_close` after a partial failure creates only what is missing.
- **Account 9000 is left out of every income statement.** `compute_profit_and_loss`, `run_accounting_report` `profit_and_loss`, `generate_annual_report_data` (9000 listed under `excluded_from_income_statement`) and `prepare_year_end_close` report the real result after the RIK close. The 9000 warning fires only for postings outside a closing entry or an amount that no longer matches the year's result.
- **Annual-report equity matches `compute_balance_sheet` in every state.** Equity shows 2970 as `current_year_result` ("Aruandeaasta kasum") plus a new `sulgemata_tulem` line (open P&L including 9000), with a `result_reconciliation` block warning when a prior year is unclosed or not transferred. ROE opening equity and prepare's balance check use the same basis.
- **Dividend legality checks count pending unconfirmed dividends.** `prepare_dividend_package` books drafts, but the ÄS § 157 checks read only confirmed journals, so two shareholders' dividends could each pass against the same retained earnings. Live unconfirmed dividend journals (`DIV-…` or crediting dividend payable) now reduce the lg 1 and lg 2 headroom and `max_net_dividend`, and are listed in the response.
- **Dividend retry with a different amount is refused.** A second call with the same `DIV-{date}-{shareholder}` key reported `duplicate` while echoing the new amounts, although the ledger kept the old journal. It now fails with `dividend_key_conflict` showing both sets of postings (also on `dry_run`); an identical retry echoes the existing journal's actual postings, and the duplicate check runs before the legality checks.
- **Dividend lg 1 ceiling includes the prior-year result.** The ceiling read only retained earnings (2960), so last year's profit closed to "Aruandeaasta kasum" (2970) or still in unclosed P&L blocked a normal dividend and pushed towards `force=true`. Distributable profit is now 2960 + 2970 (name-resolved, honouring the accounting-rules current-year-profit override) + unclosed prior-year P&L including 9000, excluding the distribution year's result, reported in `retained_earnings_check.components`. An inactive explicit `restricted_reserve_accounts` entry is accepted, and the shareholder name is desandboxed for the journal title and wrapped as untrusted text in output.
- **Recurring invoices execute exactly what was previewed.** `manage_sale_invoice` `action='recurring'` bound only the parameters, so an invoice confirmed or edited after the preview could be cloned (and with `auto_confirm` registered) unseen. The preview's source set and a canonical hash of each clone's complete create payload (client, receivable/bank account, dimensions, item accounts, VAT and amounts; `clone_payload_sha256`) are now bound into the plan; execute refuses with `plan_drift` if anything changed, clones only the reviewed ids, and rechecks each payload right before its create. Execute and its drift check read the sale invoices uncached, and each create is preceded by a live check for a clone made elsewhere meanwhile (UI or another process), which is skipped instead of duplicated.
- **Recurring invoices clone only confirmed ordinary invoices.** The default path cloned credit invoices, and explicit `invoice_ids` cloned any invoice including VOID credit notes from other months. Non-CONFIRMED, deleted and credit sources are now refused per row, ids outside `source_month` get a warning, and preview rows show `source_status`, `source_sale_invoice_type` and `source_create_date`.
- **Recurring clones keep the receivable dimension.** Clones copied `receivable_accounts_id` but dropped `receivable_accounts_dimensions_id` (and `bank_accounts_id`, `payment_description`, `subclients_id`, `recipient_clients_id`, `recipient_subclients_id`); these are now copied when present. Dedupe matches source invoice + target month, so a re-run on another day of the month no longer duplicates, and inherited recurring markers no longer pile up in notes. `confirm_error` is sandbox-wrapped.
- **`manage_sale_invoice` read `list` honours `view`.** `view="full"` was ignored.
- **Automatic import of a single `apikey*.txt` at startup works again.** It always failed with "invalid or oversized data" because it ran outside an invocation scope.
- **Inter-account transfers can no longer be booked reversed.** When both legs of a transfer carried the same stored `type` (legacy all-`C` rows), reconciliation confirmed whichever leg came first in the listing, so a transfer could be journalized in the wrong direction and the real outgoing leg deleted. Such a pair is now auto-confirmed only when exactly one leg has a signed outgoing marker; otherwise it goes to review as `direction_unresolved`. Pair confirms now post the base EUR amount, the same as one-sided confirms.
- **A signed outgoing bank row no longer settles a sale invoice.** A CAMT `DBIT` / Wise `OUT` row (e.g. a customer refund quoting the invoice reference) could exact-match and auto-confirm against an open sale invoice. Signed outgoing rows now match purchase invoices only.
- **Confirming a row whose stored type contradicts its statement direction is refused** (`stored_type_direction_mismatch`) on every confirm path, since the backend would place the cash leg on the wrong side; reconciliation execution reports carry this code (and `linked_invoice_client_mismatch`) instead of a generic `confirm_failed`. The confirm client auto-fix never writes a null client. A distribution linking invoices of several different clients is refused before any mutation (`linked_invoice_clients_ambiguous`) whether or not the transaction already has a client — one journal carries one client, so the other invoices' receipts would land in the wrong sub-ledger (v0.25.7 let such a split through when the transaction had a client); split the payment per client. An invoice without a client next to another client's invoice no longer disables the payer check.
- **CAMT import keeps a second identical entry without a bank reference.** Two genuine identical same-day entries without `AcctSvcrRef` were collapsed into one; the ledger count now decides how many copies are flagged as possible duplicates. The closing-balance tripwire now also works on the dry run (`pending_import_amount`), and execute re-reads the ledger uncached right before every row it creates (rows created earlier in the same run count too), so a row booked elsewhere during a long import stops it with `plan_drift`.
- **`analyze_unconfirmed_transactions` suggestions are safer.** Duplicate detection uses the statement direction with a ±1-day window, inter-account suggestions are skipped when the transfer is already journalized, and invoice status is read fail-closed.
- **`create_transaction` requires a positive amount**, and a Wise import stops on an unknown-outcome write instead of reporting it as skipped.
- **Receipt gross is no longer taken from the net-total line.** On a receipt like "Kokku km-ta 50,00 / KM 24% 12,00 / Kokku 62,00" the extractor picked 50 as the gross (a VAT label on the next line scored it up) and derived net 38, which still added up, so `receipt_batch` created — and in `create_and_confirm` confirmed — a 62 € receipt at 50 €. Net-labelled lines are no longer gross candidates, and a layout/text gross disagreement now blocks automatic creation (`total_gross_conflict`).
- **Receipt VAT rate comes from the receipt and its date.** The batch took the rate from the supplier's last invoice (e.g. 22% on a 2026 receipt) or none at all on the keyword path; it is now derived from extracted VAT/net and snapped to the standard or a reduced rate in force on the invoice date, with a mismatch sent to review. A reverse-charge receipt showing 0 VAT self-assesses at the standard rate in force on the invoice date instead of "-" or a stale 20/22% history rate; a reduced history rate is kept only if in force on that date, otherwise the receipt goes to review.
- **Foreign-currency invoices with `base_gross_price` no longer get phantom EUR VAT.** Base net/VAT are split in the invoice's own proportion, so a 0-VAT invoice keeps base VAT 0.
- **Purchase-invoice totals are safer.** All PATCH item paths apply the #62 structural defaults (`cl_fringe_benefits_id`, `amount`); a non-VAT company's multi-item invoice distributes the gross across items pro rata to net; the explicit-VAT rounding absorb is capped at 0.01 per item and a larger mismatch is refused instead of being hidden in the last line.
- **Duplicate purchase invoices are blocked.** `process_accounting_document` blocks an exact live supplier + invoice-number match at prepare and re-checks at create against live data (the 120 s list cache is dropped first, so an invoice booked meanwhile in the UI or by another process is caught); `create_purchase_invoice_from_pdf` refuses it too (also on live data) and now validates dates, `term_days` and non-VAT VAT fields. An explicit `allow_duplicate_invoice_number: true` (bound into the `process_accounting_document` plan) turns the block into a warning for suppliers that reuse numbers across years. Duplicate checks compare invoice numbers ignoring spaces, dashes and case, treat VOID/INVALIDATED/DELETED as not live, and limit same-amount candidates to ±7 days (`detect_duplicate_purchase_invoice` gains `invoice_date`).
- **Supplier resolution.** The business-register lookup parsed the wrong response shape and never returned data; it now reads the live autocomplete shape, requires an exact registry-code match, reports the company status and never auto-creates a deleted company. Tied supplier names resolve as ambiguous instead of silently picking the first client.
- **Receipt field extraction.** Invoice numbers must contain a digit, stay on one line, skip reference/account/IBAN/phone lines and real calendar dates (date-like numbers such as 2025-09-012 are kept), and prefer invoice labels over order labels; the invoice date prefers explicit invoice-date labels over due/delivery/order dates; a due date before the invoice date gives term 0 with a note; negated reverse-charge phrases are ignored and the phrase path requires VAT registration; dot-files are skipped in folder scans; `suggest_booking` `limit` is capped at 20.
- **Receipt-batch invoice notes no longer contain the source filename.**
- **Bank classification never books a refund as an expense.** Counterparty groups mixed incoming and outgoing rows and were categorized from the first row only, so an incoming refund could be booked as a new purchase invoice. Groups are now split by direction and the apply step skips non-outgoing rows.
- **Payment-receipt invoice cross-references match invoice numbers ignoring spaces and dash variants**, like the duplicate checks.
- **Wise import no longer double-books an own transfer already journalized from the other bank.** The inter-account check now uses BookingGuard Lane B with a ±1-day window at plan and execute time (it matched the exact date only, and a journal labelled with another reference never matched). A differently labelled or ambiguous same-amount journal sends the row to review instead of auto-confirming; an already-journalized transfer no longer creates a duplicate PROJECT bank row.
- **Wise cross-currency own transfers are no longer confirmed in the foreign nominal.** A USD → EUR own transfer posted 100 to the EUR bank where 92 EUR arrived. Transfers that are not same-currency EUR are imported unconfirmed with a `wise_transfer_cross_currency` review; the base-EUR booking is left to `reconcile_inter_account_transfers`.
- **Wise own-account transfers are no longer skipped as jar transfers.** `TRANSFER-*` / `BANK_DETAILS_*` rows whose two endpoints carry the same name went to "Jar / self-transfer"; they now flow through the ownership / inter-account path. Naming an already-imported row in `confirm_own_transfer_ids` produces a `wise_transfer_already_imported` review instead of silently doing nothing.
- **Wise bank rows stay within the API limits** (description 150, `bank_account_name` 100) while keeping the `WISE:{id}` tag and the direction marker; duplicate detection still recognises rows imported before the caps. A row whose Wise ID is too long for the tag and marker to fit (over 122 characters, or over 102 with a fee) is skipped with a reason instead of being truncated. A fully imported re-run no longer fails with `wise_client_not_found` or a missing fee dimension, and raw error text in result `status` fields is sandboxed.
- **Lightyear sells book the cash the statement shows.** `book_lightyear_trades` debited the broker with the capital-gains file's proceeds, so a EUR sell overstated the broker by the proceeds gap and FX sells deducted the fees twice. Sells now debit the broker with the cash actually received (net for EUR sells, the EUR conversion leg for FX sells), expense the trade/FX fees, credit the investment at Lightyear's FIFO cost basis and book the gain/loss as the balancing figure; gains rows are matched on the sell's consideration, and a gain differing from Lightyear's `Capital Gains (EUR)` is shown as `lightyear_capital_gains_eur`.
- **Lightyear booking no longer promises journals the API rejects.** Every projected posting is checked against the dimension rules on dry run and execute; a 4xx client rejection is reported as a failed command (`journal_create_rejected`) with the sandboxed upstream detail instead of `mutation_outcome_unknown` (a 5xx, timeout or network error stays `mutation_outcome_unknown`, since the journal may have been saved), and the approval view shows each posting's `accounts_dimensions_id`, including an auto-filled single dimension. Rows with an impossible date (`31/02/2026`, non-DD/MM/YYYY) go to review (`invalid_date`) and ISO timestamps book on their calendar day; same-reference rows with differing content go to review (`reference_conflict`) instead of silently booking the first; `parse_lightyear_statement` cash reconciliation counts buys and conversion source legs at gross; a `capital_gains_file_ref` no longer triggers the missing-file warning.
- **`accounting_inbox` dry runs no longer exhaust the plan store.** Each dry run minted consume-once plan handles for every CAMT, Wise and inter-account preview (one slot per file per run) until gated flows failed with `plan_capacity_exceeded`. Its approvable previews now point at the tool's own dry run, which returns the required `plan_handle`, instead of an execute card that always failed with `plan_handle_required`.
- **`accounting_inbox` reports Wise transfer reviews.** Ownership, dimension, cross-currency, ambiguous-journal and already-imported reviews each become a review item with a stable id, advisory invoice FX corrections are listed, and a Wise import with open reviews is no longer offered for approval (the inbox reported it clean). A skipped (not failed) import step now defers the ledger-reading steps as `materialization_state: "pending_input"` instead of reporting an earlier step as failed.
- **Guided workflow continuation works.** A pending question in `workflow_action_v2` routed to `get_setup_instructions`; it now tells the agent to ask the user and rerun `accounting_inbox` with the answer, naming the input when known (e.g. `answer_input: "bank_account_dimension_id"`); question items keep their item id, unanswered questions appear as `needs_input` blockers, and a completed workflow carries no `next_action`. `continue_accounting_workflow action="next"` implements the advertised `workflow_handle` + `item_id` continuation for review items (the unused `answer` input was removed), refuses a compact v2 response instead of falsely reporting "no workflow action pending", and sandboxes echoed caller text.
- **Owner-expense reimbursements are not booked twice on retry.** Booking (tool or continuation) returns the existing journal as `booking_status: "duplicate"` when a live journal with the same document number, owner, date and total exists; `prepare_action` no longer mints a plan handle while the item has unresolved questions (`status: "needs_answers"`), and the echoed recommendation and compliance basis are sandboxed.
- **Uploaded documents keep a meaningful name.** `process_accounting_document` uploaded every source as `document.pdf` and base64 uploads got a random UUID name; the source file's own name is used, and `attach_document` / `create_purchase_invoice_from_pdf` take an optional `file_name`. Upload names go through one sanitizer that strips control characters and path components but keeps Estonian letters (receipt-inbox uploads sent raw directory-entry names). A failed snapshot write no longer leaks a temp directory.
- **A `.reclaim` guard left by a crashed process no longer blocks lock recovery forever** (audit-log and booking-guard locks); it is cleared with the same dead-owner check.

### Added

- **`confirm_transaction` checks inter-account confirms for duplicates.** Confirming against an own bank account now looks for an existing transfer journal (±1 day) and a matching bank posting, warns by default, and refuses with `block_on_duplicate: true`; a failed read in this advisory check becomes a warning, never a failed confirm.
- **Lightyear `interest_account` and `fund_distribution_account`.** `book_lightyear_distributions` routes Interest rows to `interest_account` (default 8400 "Intressitulu hoiustelt") and fund Distribution rows to `fund_distribution_account` (default 8320), both name-resolved; `income_account` now applies to Dividend rows only (all three types used to land on one account).

### Changed

- **`generate_annual_report_data` `balance_sheet.equity.current_year_result.amount` changed meaning.** It used to be the year's net profit; it is now the balance of 2970 "Aruandeaasta kasum" (0 before the RIK close), with the not-yet-closed result shown separately as `sulgemata_tulem`. Read the year's profit from the income statement instead.
- **Year-end close follows RIK's e-arveldaja method.** `prepare_year_end_close` / `execute_year_end_close` now propose and book the two manual entries from RIK's guide "Äriühingu majandusaasta lõpetamiskanded e-arveldajas": Dec 31 profit D 9000 "Arvestuslik koondtulemus" / K 2970 "Aruandeaasta kasum" (loss reversed, document `YEC-RESULT-YYYY`) and Jan 1 of the next year D 2970 / K 2960 "Jaotamata kasum" (document `YEC-RETAINED-YYYY`, optional `reserve_capital_amount` split to 2940). Revenue and expense accounts are no longer zeroed: the old `YECL-YYYY` entry zeroed every P&L account into 2970, which broke e-arveldaja's own income statement, double-credited 2970 when the RIK entry was also booked, and never moved 2970 to 2960. Accounts are name-resolved with standard numbers as fallback.
- **`force=true` in `prepare_dividend_package` no longer overrides the ledger-imbalance block**; it only overrides the two ÄS § 157 clauses.
- **Dependencies updated.** `@modelcontextprotocol/sdk` 1.30.0 → 1.30.1, `@llamaindex/liteparse` 2.14.6 → 2.14.7, `dotenv` 17.4.2 → 18.0.3, plus dev tooling (`@babel/parser` 8.0.6, `@types/node` 26.6.2, `tsx` 4.23.15, `vite` 8.3.1). Declared floors raised to the tested versions. dotenv 18 removes preloading, `.env.vault` and console tips, and adds an opt-in fast parser; this server only calls `dotenv.parse()` with the default parser, so behaviour is unchanged. `vitest` stays on 4.1.11 because vitest 5 requires Node ≥ 22.12 and CI still runs unit tests on Node 20.
- **`attach_document` no longer silently replaces an existing source document.** It refuses with `document_exists` (showing the current file name, sandboxed, and the target record) unless `replace_existing: true` is passed; the tool is now annotated and catalogued as destructive, a replacement is audited with the replaced file name, and `file_path` must be absolute (or `base64:[<ext>:]<data>`).
- **Wise import no longer modifies confirmed purchase invoices.** The Wise-rate lock and the legacy EUR settlement fix PATCHed money fields on CONFIRMED invoices without items (bypassing the confirmed-record rules); they are now reported as advisories with `proposed_correction` (the upstream invoice number is sandboxed), and purchase invoices are no longer part of the Wise plan digest.

### Security

- **Credential writes can no longer be captured through a planted symlink.** `.env` and other secret files were written via a predictable `<file>.tmp-<pid>` path opened without `O_EXCL`, so a symlink planted there received the API password. The temp file now has a random name, is opened `O_EXCL` with mode 0600, and cleanup never follows a symlink.
- **Opening and statement balances are bound to the connection.** When several companies share one accounting-rules bundle (`EARVELDAJA_RULES_DIR` or a legacy root bundle), `opening-balances.json` and `statement-balances.json` were shared too, so one company's algbilanss was folded into another's balances and ÄS § 157 checks. Records are now stamped with the connection fingerprint; a record from another connection is ignored with a warning (re-import after an API key rotation). Legacy unstamped records keep applying on single-connection servers and in the default per-connection bundle directory; they are ignored only in a shared bundle with several connections. Both files are written atomically with mode 0600 and reads are capped at 1 MB.
- **Nested sandbox markers are stripped completely** before text is persisted or matched.
- **Duplicate connection names are made unique** (`apikey.txt` + `apikey.TXT` → `apikey`, `apikey-2`), so the multi-company `connection` guard cannot match two companies.
- **`.env` values always round-trip.** Quote-wrapped values and a literal `\n` were rewritten corrupted (a stored password could silently stop working); every value is now checked with `dotenv.parse` before writing, and unrepresentable values are refused.
- **Accounting-rules bundle files are created 0600 and directories 0700.**
- **`clear_session_log` leaves a tombstone entry** (action `LOG_CLEARED`, filterable in `get_session_log`; time, connection, process, entries removed) and reports failure instead of claiming success; the tombstone replaces the log atomically, so a failed clear leaves the existing entries intact. Failed audit-log writes and slow audit-lock waits are reported on stderr.
- **More bank free text is sandboxed:** CAMT end-to-end id, counterparty IBAN and registry code, reference and patch fields, and journal `document_number` in duplicate warnings.
- **Invoice numbers are sandboxed in `detect_duplicate_purchase_invoice` output.**
- **Document content must match its extension.** PDF/PNG/JPEG uploads must carry their format's magic bytes: a local file whose content does not match its extension (`content_type_mismatch`) or a `base64:pdf|png|jpg:` payload without the signature is refused. XML/CSV/TXT inputs are unaffected.
