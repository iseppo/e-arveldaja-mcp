...
- **Source-reference idempotency.** Bank `ref_number` is canonicalized at the single write boundary and capped at 20 chars (the maintainer-reported backend limit — a fallback assumption, not demo-probe-confirmed); when truncated, the full reference is woven into `description` (before any trailing source-direction / camt marker so end-anchored metadata regexes still match) so it is never lost, and Wise import canonicalizes the reference inside its transaction signature so an over-cap row still dedupes against its previously-stored truncated transaction. `create_journal` now emits an advisory warning when its `document_number` already exists on a live journal (creation still proceeds).
